...
The active Phase 3.1 recipe preview imagery implementation is in progress on `codex/init-001-recipe-preview-images` as of 2026-06-17. The branch adds an async/cache-backed recipe image resolver, the `recipe_image_cache` schema metadata, Replit App Storage object storage, strict title/core-ingredient cache matching, an image accuracy judge before approval, all-or-none client hydration into the existing Ticket Pass/Prep Tray `imageUrl` slots, and a curated fixture seed path for pre-generated library entries. Runtime generation is explicitly gated behind `RECIPE_IMAGE_GENERATION_ENABLED=true`; cache hits can still serve while generation is disabled. Replit live validation generated one three-recipe image set with OpenAI `gpt-image-2` low quality and confirmed cached Ticket Pass plus Prep Tray loading at runtime head `76b998d`. Wilson's follow-up Refresh Suggestions smoke showed the original late image pop-in felt wrong even when technically correct, so the branch now parallelizes the three image jobs, defaults runtime output to compressed JPEG, polls restored/fresh batches every 10 seconds without increasing request count, keeps the current image-backed tickets visible during Refresh until the replacement set is ready or deliberately falls back, and charges recipe-image abuse limits only when generation starts/restarts rather than on every pending poll. EFF-022 is related but unchanged because this branch does not alter recipe prompt cuisine behavior.
...
| #192 | Open | `codex/init-001-recipe-preview-images` | Phase 3.1 async recipe preview imagery pipeline; local checks passed at runtime head `76b998d`, Replit schema/App Storage/live-provider/cache-loading validation passed at `76b998d`, and earlier GitHub `unit`, `e2e_guest_smoke`, `npm-audit`, `trufflehog_pr`, and CodeQL passed before the final polling-window follow-up. Final PR-head CI must pass again after the latest pushes before merge readiness |
...
1. Finish PR #192 for the active Phase 3.1 recipe preview imagery branch `codex/init-001-recipe-preview-images`: final PR-head CI must pass after the refresh-latency follow-up, and Replit should run one focused smoke at the new head to confirm compressed-JPEG parallel generation, Refresh Suggestions replacement timing, and cached Prep Tray loading. The PR/handoff should retain the 2026-06-17 Replit evidence from runtime head `76b998d` (schema/App Storage smoke, one real OpenAI `gpt-image-2` low three-image set, all-or-none cached Ticket Pass reveal, Prep Tray image loading, latency/cost/negative scope) and clearly mark the new refresh-latency behavior as unit-tested pending live smoke. After imagery, the next documented Phase 3.1 candidates are light Prep Tray shell alignment if needed, ingredient chip unification, or closeout visual review.
