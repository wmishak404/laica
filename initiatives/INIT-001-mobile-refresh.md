...
The active Phase 3.1 recipe preview imagery implementation is in progress on `codex/init-001-recipe-preview-images` as of 2026-06-18. The branch adds an async/cache-backed recipe image resolver, the `recipe_image_cache` schema metadata, Replit App Storage object storage, strict title/core-ingredient cache matching, an image accuracy judge before approval, and a curated fixture seed path for pre-generated library entries. Runtime generation is explicitly gated behind `RECIPE_IMAGE_GENERATION_ENABLED=true`; cache hits can still serve while generation is disabled. Replit live validation generated one three-recipe image set with OpenAI `gpt-image-2` low quality and confirmed cached Ticket Pass plus Prep Tray loading at runtime head `76b998d`, but Wilson's follow-up Refresh Suggestions smoke showed 80-120 second late image arrival was the wrong UX even when technically fair. The branch now pivots the user-visible runtime: Ticket Pass stays placeholder-only and does not call an image resolver, while Prep Tray calls a new selected-recipe resolver for one image, shows a subtle spinner in the placeholder while the preview is being prepared, and keeps cooking non-blocking. Replit selected-image validation at PR head `9e62f0f` confirmed that an approved selected image appears in Prep Tray and does not bleed back into Ticket Pass after returning or resuming the Chef It Up session. A separate reset/remount to the main menu was observed during that smoke, but session restore recovered the same suggestions; fuller active-flow reload restoration is filed as high-priority EFF-027 and remains outside PR #192 unless it becomes imagery-specific. Visible polling stops when the user leaves Prep Tray, refreshes recipes, starts cooking, or unmounts the flow. Gemini/Nano Banana support is implemented behind `RECIPE_IMAGE_PROVIDER=gemini` for Replit benchmarking with `GEMINI_API_KEY`; OpenAI remains the default provider until benchmark evidence justifies switching. EFF-022 is related but unchanged because this branch does not alter recipe prompt cuisine behavior.
...
| Phase 3.1 | In progress | [#69](https://github.com/wmishak404/laica/pull/69), [#73](https://github.com/wmishak404/laica/pull/73), [#75](https://github.com/wmishak404/laica/pull/75), [#81](https://github.com/wmishak404/laica/pull/81), [#141](https://github.com/wmishak404/laica/pull/141), [#175](https://github.com/wmishak404/laica/pull/175), [#184](https://github.com/wmishak404/laica/pull/184); [#78](https://github.com/wmishak404/laica/pull/78) closed unmerged; active branch `codex/init-001-recipe-preview-images` / [#192](https://github.com/wmishak404/laica/pull/192) | Kickoff/audit, Planning copy/count emphasis, Slop Bowl pantry-check chip alignment, Setup/Settings inventory review chip states, Slop Bowl generated-result button typography alignment, PR #78 abandonment/retry brief, accepted Ticket Pass hierarchy retry, and Planning toast cleanup are merged. Recipe preview imagery passed Replit schema/App Storage/live-provider/cache-loading validation at runtime head `76b998d`; final PR-head selected Prep Tray image behavior and Gemini benchmark smoke remain pending. Remaining Phase 3.1 scope after imagery is light Prep Tray alignment if needed, ingredient chip unification, and closeout visual review |
...
1. Finish PR #192 for the active Phase 3.1 recipe preview imagery branch `codex/init-001-recipe-preview-images`: final PR-head CI must pass after the latest evidence/docs commits, and no additional Replit selected-image smoke is needed unless runtime files change after `9e62f0f`. The PR/handoff should retain the 2026-06-17 Replit evidence from runtime head `76b998d` (schema/App Storage smoke, one real OpenAI `gpt-image-2` low three-image set, all-or-none cached Ticket Pass reveal, Prep Tray image loading, latency/cost/negative scope) as historical pipeline evidence, and the 2026-06-18 Replit smoke from `9e62f0f` as the accepted user-visible selected-image behavior: Ticket Pass placeholder-only, Prep Tray selected image, and no image bleed-back when returning to Ticket Pass. Gemini/OpenAI benchmark timings remain provider-comparison follow-up, not a merge blocker while OpenAI stays the default. After imagery, the next documented Phase 3.1 candidates are light Prep Tray shell alignment if needed, ingredient chip unification, or closeout visual review.
