...
4. Continue EPIC-021 on `codex/epic-021-scan-upload-implementation` for scan upload runtime follow-through. The current slice has local unit/type/build/dev-server/browser-smoke validation plus bounded-concurrency coverage, and Wilson's final Replit check passed at `ef28e59`. Provider-level multi-image batching and final adaptive chunk thresholds remain follow-up scope unless explicitly pulled into this PR.
