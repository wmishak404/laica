...
| Phase 4 | In progress for narrow audio lifecycle slice | `codex/init-001-cooking-audio-cleanup` | Back/Finish/unmount audio cleanup branch active; full cooking guidance, Ready Check, Coach Feed, timers, inline recovery, and Finish/Phase 5 semantics remain planned |
...
- Replit validation of INIT-003 on 2026-05-26 exposed a Phase 4 acceptance item: if voice synthesis/playback has started, Back to Planning or any other cooking-guide exit must stop active/queued audio so speech cannot continue after leaving the cooking surface. Branch `codex/init-001-cooking-audio-cleanup` implements this item as a narrow lifecycle slice; it still needs PR review and exact-head CI before merge.
