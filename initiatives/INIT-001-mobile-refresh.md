...
**Created:** 2026-04-29
**Current phase:** Phase 3.1 follow-up / Phase 4 follow-up
...
| Phase 3.1 | In progress | [#69](https://github.com/wmishak404/laica/pull/69), [#73](https://github.com/wmishak404/laica/pull/73), [#75](https://github.com/wmishak404/laica/pull/75), [#81](https://github.com/wmishak404/laica/pull/81), [#141](https://github.com/wmishak404/laica/pull/141), [#175](https://github.com/wmishak404/laica/pull/175), [#184](https://github.com/wmishak404/laica/pull/184), [#192](https://github.com/wmishak404/laica/pull/192), [#201](https://github.com/wmishak404/laica/pull/201), [#208](https://github.com/wmishak404/laica/pull/208), [#234](https://github.com/wmishak404/laica/pull/234); [#78](https://github.com/wmishak404/laica/pull/78) closed unmerged | Kickoff/audit, Planning copy/count emphasis, Slop Bowl pantry-check chip alignment, Setup/Settings inventory review chip states, Slop Bowl generated-result button typography alignment, PR #78 abandonment/retry brief, accepted Ticket Pass hierarchy retry, Planning toast cleanup, selected Prep Tray recipe imagery, Chef It Up reload restoration, Prep Tray ready-image hero fill, and checked Ticket Pass/Prep Tray ingredient chips are merged. Gemini benchmarking remains a Phase 3.1 provider-comparison follow-up before any provider-default change. Remaining Phase 3.1 scope is provider comparison if needed and closeout visual review |
...
| Phase 4 | In progress after speech arbitration and step-recovery slices | [#191](https://github.com/wmishak404/laica/pull/191) merged as `104ee0c`; [#236](https://github.com/wmishak404/laica/pull/236) merged as `f3e886b`; [#256](https://github.com/wmishak404/laica/pull/256) merged as `f40cb1c` | Back/Finish/unmount cleanup plus Wilson's speech-action matrix are merged with focused deterministic coverage, Wilson's 12-case Replit pass, and exact-head GitHub `unit` / `e2e_guest_smoke` at `b2e6f54`. PR #236 adds inline retry/basic-backup handling for failed or empty-array step generation and corrects current Finish completion so it sends no hidden rating or pantry-update claim; exact-head GitHub checks passed at `3053faa`, with human Replit validation deferred to the next production/release batch. PR #256 rejects blank/placeholder generated or restored instructions before entering/saving Live Cooking; exact-head GitHub checks passed at `bb98cf8`, with human Replit validation deferred to the same release/batch lane. Full Ready Check, Coach Feed, timer redesign, provider prompts, schema, and Phase 5 cleanup remain planned |
...
| #236 | Merged as `f3e886b` | `codex/init-001-cooking-step-recovery` | Current Phase 4 slice: Live Cooking shows inline recovery with Retry/basic-backup choices when cooking-step generation fails or returns an empty step array, exposes final-step `Finish`, and omits hidden rating/notes plus pantry-update copy on linked completion. Focused unit coverage is in `tests/unit/live-cooking-guest-session.test.tsx`; exact-head GitHub `unit`, `e2e_guest_smoke`, `npm-audit`, `trufflehog_pr`, and CodeQL passed at `3053faa`. Human Replit validation is deferred to the next production/release batch, and blank/non-cookable generated-step validation remains required for the full Phase 4 revamp. No Ready Check, Coach Feed, timer redesign, provider prompt, schema, or Phase 5 cleanup changes |
| [#256](https://github.com/wmishak404/laica/pull/256) | Merged as `f40cb1c` | `codex/init-001-cooking-step-validation` | Phase 4 generated-step validation slice: blank, whitespace-only, and obvious placeholder provider output stays in inline recovery instead of rendering Step 1 or starting a linked cooking session; saved placeholder step trays regenerate rather than restoring as ready. Focused coverage is in `tests/unit/live-cooking-guest-session.test.tsx`. Exact-head GitHub `unit`, `e2e_guest_smoke`, `npm-audit`, `trufflehog_pr`, and CodeQL passed at `bb98cf8` before merge. Human Replit validation is deferred to release/batch validation. No Ready Check, Coach Feed, timer, prompt/provider, schema, speech/audio, or Phase 5 cleanup changes |
