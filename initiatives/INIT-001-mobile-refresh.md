...
2. Run Replit validation for authenticated Planning entry, Chef It Up time/cuisine/staple-check flow, multi-cuisine staple representation, concrete herb staple labels, confirmed-staple pantry save, recipe generation without 400 preference-length failures, refresh suggestions, Ticket Pass selection, Ticket Pass stable-order expansion across all three recipes, Prep Tray -> Cooking, Slop Bowl quick-add/remove, sparse-pantry guard, Slop Bowl generation, and Slop Bowl -> Edit pantry. If Replit shows a first-load `useState` null crash after pulling, clear `node_modules/.vite`, refresh dependencies, and restart before continuing validation.
