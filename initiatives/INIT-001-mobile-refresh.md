...
| Phase 4 | In progress after speech arbitration and step-recovery slices | [#191](https://github.com/wmishak404/laica/pull/191) merged as `104ee0c`; [#236](https://github.com/wmishak404/laica/pull/236) merged as `f3e886b` | Back/Finish/unmount cleanup plus Wilson's speech-action matrix are merged with focused deterministic coverage, Wilson's 12-case Replit pass, and exact-head GitHub `unit` / `e2e_guest_smoke` at `b2e6f54`. PR #236 adds inline retry/basic-backup handling for failed or empty-array step generation and corrects current Finish completion so it sends no hidden rating or pantry-update claim; exact-head GitHub checks passed at `3053faa`, with human Replit validation deferred to the next production/release batch. The full Phase 4 revamp must also reject blank/non-cookable generated instructions before entering or saving Live Cooking state. Full Ready Check, Coach Feed, timer redesign, provider prompts, schema, and Phase 5 cleanup remain planned |
