...
- The active Phase 3.1 recipe preview imagery branch `codex/init-001-recipe-preview-images` passed local `npm run check`, `npx vitest run tests/unit/recipe-images.test.ts tests/unit/recipe-image-route.test.ts tests/unit/meal-planning.test.tsx`, full `npm run test:unit` (42 files / 277 tests), and `npm run build` on 2026-06-16. A provider-light Playwright test was added for complete-set image reveal, but local execution was not reached because `@dotenvx/dotenvx` is not installed in the worktree and fetching/executing it with decrypted secrets was rejected by the sandbox for secret-exfiltration risk. Replit/App Storage live validation remains required before enabling runtime image generation or claiming merge readiness.
