...
| #192 | Open | `codex/init-001-recipe-preview-images` | Phase 3.1 recipe imagery pivot: Ticket Pass placeholder-only, selected Prep Tray image resolver with subtle placeholder spinner and non-blocking cooking, and Gemini provider/benchmark path behind `RECIPE_IMAGE_PROVIDER=gemini`; Replit schema/App Storage/live OpenAI cache validation passed earlier at `76b998d`, while final PR-head CI and Replit selected-image/Gemini benchmark smoke remain pending |
...
1. Finish PR #192 for the active Phase 3.1 recipe preview imagery branch `codex/init-001-recipe-preview-images`: final PR-head CI must pass after the selected-image pivot, and Replit should run one focused smoke at the new head to confirm Ticket Pass is placeholder-only with no resolver polling, Prep Tray requests one selected image only, the placeholder shows a subtle spinner while pending, visible polling stops when the user leaves or starts cooking, and Gemini/OpenAI benchmark timings are recorded. The PR/handoff should retain the 2026-06-17 Replit evidence from runtime head `76b998d` (schema/App Storage smoke, one real OpenAI `gpt-image-2` low three-image set, all-or-none cached Ticket Pass reveal, Prep Tray image loading, latency/cost/negative scope) as historical pipeline evidence, but clearly mark the user-visible runtime as selected Prep Tray imagery only. After imagery, the next documented Phase 3.1 candidates are light Prep Tray shell alignment if needed, ingredient chip unification, or closeout visual review.
