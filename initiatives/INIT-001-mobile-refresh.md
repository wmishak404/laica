...
Draft PR #191 is the current Phase 4 speech arbitration cleanup on `codex/init-001-cooking-audio-cleanup`. It began with the Replit-observed case where speech playback could continue after leaving the cooking guide. The exit slice shares Back/Finish/unmount cleanup, cancels delayed speech and retry timers, ignores late synthesis responses, stops browser speech synthesis, and cancels active voice recording. Wilson's 2026-06-17 follow-up clarified that merge readiness also requires the existing Live Cooking speech-action matrix: initial Step 1 audio, Next/Previous interruption, competing speech actions, Ask for Help stop-before-recording, Back/Finish/unmount cleanup, immediate/persistent mute, unmute-no-autoplay, transcript fidelity, rapid repeated actions, and timer interruption. This remains bounded to existing Live Cooking speech controls and does not widen into Ready Check, Coach Feed redesign, prompt/provider changes, schema, or Phase 5 completion semantics.
...
| Phase 4 | In progress for narrow audio lifecycle slice | [#191](https://github.com/wmishak404/laica/pull/191) / `codex/init-001-cooking-audio-cleanup` | Back/Finish/unmount audio cleanup branch active; full cooking guidance, Ready Check, Coach Feed, timers, inline recovery, and Finish/Phase 5 semantics remain planned |
...
3. Review/merge [PR #191](https://github.com/wmishak404/laica/pull/191), the narrow Phase 4 audio lifecycle cleanup, before treating the Replit-observed speech-leak acceptance item as closed. Full Phase 4 still starts from fresh `origin/main` when cooking guidance begins; it owns Ready Check, Coach Feed, timers, live-cooking inline AI error recovery, and Finish semantics that EFF-018 intentionally deferred.
...
- Replit validation of INIT-003 on 2026-05-26 exposed a Phase 4 acceptance item: if voice synthesis/playback has started, Back to Planning or any other cooking-guide exit must stop active/queued audio so speech cannot continue after leaving the cooking surface. [PR #191](https://github.com/wmishak404/laica/pull/191) implements this item as a narrow lifecycle slice; exact-head automated evidence has passed and the remaining action is Wilson review/merge.
