...
1. Continue Phase 3.1 after PR #192's recipe imagery merge (`efd1f88`) by choosing the next product slice: Gemini/OpenAI provider benchmark comparison before any provider-default change, light Prep Tray shell/image-area alignment if needed, ingredient chip unification, or closeout visual review. Keep the 2026-06-17 Replit evidence from runtime head `76b998d` as historical pipeline evidence, and the 2026-06-18 Replit smoke from `9e62f0f` as the accepted user-visible selected-image behavior: Ticket Pass placeholder-only, Prep Tray selected image, and no image bleed-back when returning to Ticket Pass. High-priority EFF-027 separately owns direct active-flow reload restoration after unexpected remounts.
