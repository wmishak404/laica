...
| Phase 4 | In progress for speech arbitration slice | [#191](https://github.com/wmishak404/laica/pull/191) / `codex/init-001-cooking-audio-cleanup` | Draft branch: Back/Finish/unmount cleanup implemented, but Wilson's speech-action matrix must be converted from TODO tests into passing assertions before merge readiness; full cooking guidance, Ready Check, Coach Feed, timer redesign, inline recovery, and Finish/Phase 5 semantics remain planned |
...
3. Continue draft [PR #191](https://github.com/wmishak404/laica/pull/191) as the existing Live Cooking speech arbitration slice, not a broader Phase 4 redesign. Before it becomes merge-ready, convert the TODO cases in `tests/unit/live-cooking-guest-session.test.tsx` into passing assertions for initial Step 1 speech, step/action interruption, Ask for Help stop-before-recording, Back/Finish/unmount cleanup, mute persistence, unmute-no-autoplay, transcript fidelity, and rapid action settling. Full Phase 4 still starts from fresh `origin/main` when cooking guidance begins; it owns Ready Check, Coach Feed, timer redesign, live-cooking inline AI error recovery, and Finish semantics that EFF-018 intentionally deferred.
