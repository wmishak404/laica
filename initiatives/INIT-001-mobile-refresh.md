...
**Active PR:** [#191](https://github.com/wmishak404/laica/pull/191) draft
...
The current INIT-001 branch is the Phase 4 speech arbitration cleanup in draft [PR #191](https://github.com/wmishak404/laica/pull/191) / `codex/init-001-cooking-audio-cleanup`. It began as the narrow Replit-observed case where speech playback could continue after leaving the cooking guide. Wilson's 2026-06-17 follow-up clarified the actual merge bar: the existing Live Cooking controls need a speech-action matrix before merge, covering initial Step 1 audio, Next/Previous interruption, Repeat/timer/assistance speech conflicts, Ask for Help stop-before-recording, Back/Finish/unmount exit cleanup, immediate/persistent mute, unmute-no-autoplay, transcript fidelity, rapid repeated actions, and timer interruption. The 2026-06-18 PR update implements that matrix with current-request speech arbitration and passing deterministic coverage. Local focused/unit/check/build/audit/diff gates pass at the current PR branch head, but the automated E2E lane is blocked by local validation issues outside this Live Cooking patch: the decrypted DB is missing `anonymous_recipe_usage`, and the existing landing accessibility guardrail reports auth-control contrast failures. Remaining PR #191 work is E2E validation-lane resolution or Wilson validation decision, non-draft GitHub checks, Wilson's merge decision, and post-merge closeout. This keeps the branch inside existing Live Cooking audio behavior and does not widen it into Ready Check, Coach Feed redesign, prompt/provider changes, schema, or Phase 5 completion semantics.
...
| Phase 4 | In progress for speech arbitration slice | [#191](https://github.com/wmishak404/laica/pull/191) / `codex/init-001-cooking-audio-cleanup` | Draft branch: Back/Finish/unmount cleanup plus Wilson's speech-action matrix are implemented with passing focused deterministic coverage; local focused/unit/check/build/audit/diff gates pass at the current PR branch head, but automated E2E is blocked by local DB schema drift and existing landing contrast failures. Full cooking guidance, Ready Check, Coach Feed, timer redesign, inline recovery, and Finish/Phase 5 semantics remain planned |
...
3. Continue draft [PR #191](https://github.com/wmishak404/laica/pull/191) as the existing Live Cooking speech arbitration slice, not a broader Phase 4 redesign. The acceptance cases in `tests/unit/live-cooking-guest-session.test.tsx` now cover initial Step 1 speech, step/action interruption, Ask for Help stop-before-recording, Back/Finish/unmount cleanup, mute persistence, unmute-no-autoplay, transcript fidelity, rapid action settling, and timer interruption. Remaining PR #191 action is E2E validation-lane resolution or Wilson validation decision, non-draft GitHub checks, Wilson's merge decision, and post-merge closeout. Full Phase 4 still starts from fresh `origin/main` when cooking guidance begins; it owns Ready Check, Coach Feed, timer redesign, live-cooking inline AI error recovery, and Finish semantics that EFF-018 intentionally deferred.
