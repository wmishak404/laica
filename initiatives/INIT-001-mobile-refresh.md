...
[PR #256](https://github.com/wmishak404/laica/pull/256) continues that same Phase 4 recovery lane. It keeps cooks out of a bogus Step 1 when the generated output contains only blank, whitespace-only, or obvious placeholder instructions, and it prevents linked cooking sessions from starting from those invalid steps. It also prevents browser-local saved placeholder step trays from restoring as ready; the app regenerates instead. This branch intentionally stays inside current Live Cooking validation/recovery and does not start Ready Check, Coach Feed, timers, prompt/provider changes, schema work, or Phase 5 cleanup semantics.
...
| Phase 4 | In progress after speech arbitration and step-recovery slices | [#191](https://github.com/wmishak404/laica/pull/191) merged as `104ee0c`; [#236](https://github.com/wmishak404/laica/pull/236) merged as `f3e886b`; [#256](https://github.com/wmishak404/laica/pull/256) active | Back/Finish/unmount cleanup plus Wilson's speech-action matrix are merged with focused deterministic coverage, Wilson's 12-case Replit pass, and exact-head GitHub `unit` / `e2e_guest_smoke` at `b2e6f54`. PR #236 adds inline retry/basic-backup handling for failed or empty-array step generation and corrects current Finish completion so it sends no hidden rating or pantry-update claim; exact-head GitHub checks passed at `3053faa`, with human Replit validation deferred to the next production/release batch. The active 2026-07-06 validation slice rejects blank/placeholder generated or restored instructions before entering/saving Live Cooking. Full Ready Check, Coach Feed, timer redesign, provider prompts, schema, and Phase 5 cleanup remain planned |
...
| [#256](https://github.com/wmishak404/laica/pull/256) | Open | `codex/init-001-cooking-step-validation` | Phase 4 generated-step validation slice: blank, whitespace-only, and obvious placeholder provider output stays in inline recovery instead of rendering Step 1 or starting a linked cooking session; saved placeholder step trays regenerate rather than restoring as ready. Focused coverage is in `tests/unit/live-cooking-guest-session.test.tsx`. No Ready Check, Coach Feed, timer, prompt/provider, schema, speech/audio, or Phase 5 cleanup changes |
