...
| Phase 4 | In progress after speech arbitration slice | [#191](https://github.com/wmishak404/laica/pull/191) merged as `104ee0c`; [#236](https://github.com/wmishak404/laica/pull/236) ready for review | Back/Finish/unmount cleanup plus Wilson's speech-action matrix are merged with focused deterministic coverage, Wilson's 12-case Replit pass, and exact-head GitHub `unit` / `e2e_guest_smoke` at `b2e6f54`. PR #236 adds inline retry/basic-backup handling for failed step generation and corrects current Finish completion so it sends no hidden rating or pantry-update claim. Full Ready Check, Coach Feed, timer redesign, provider prompts, schema, and Phase 5 cleanup remain planned |
...
| #236 | Ready for review | `codex/init-001-cooking-step-recovery` | Current Phase 4 slice: Live Cooking shows inline recovery with Retry/basic-backup choices when cooking-step generation fails or returns no usable steps, exposes final-step `Finish`, and omits hidden rating/notes plus pantry-update copy on linked completion. Focused unit coverage is in `tests/unit/live-cooking-guest-session.test.tsx`; no Ready Check, Coach Feed, timer redesign, provider prompt, schema, or Phase 5 cleanup changes |
