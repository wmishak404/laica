...
1. Finish PR #192 for the active Phase 3.1 recipe preview imagery branch `codex/init-001-recipe-preview-images`: final PR-head CI must pass after the polling-window follow-up, and the PR/handoff should carry the 2026-06-17 Replit evidence from runtime head `76b998d` (schema/App Storage smoke, one real OpenAI `gpt-image-2` low three-image set, all-or-none cached Ticket Pass reveal, Prep Tray image loading, latency/cost/negative scope). After imagery, the next documented Phase 3.1 candidates are light Prep Tray shell alignment if needed, ingredient chip unification, or closeout visual review.
