...
| Phase 4 | In progress after speech arbitration slice | [#191](https://github.com/wmishak404/laica/pull/191) merged as `104ee0c`; [#236](https://github.com/wmishak404/laica/pull/236) ready for review | Back/Finish/unmount cleanup plus Wilson's speech-action matrix are merged with focused deterministic coverage, Wilson's 12-case Replit pass, and exact-head GitHub `unit` / `e2e_guest_smoke` at `b2e6f54`. PR #236 adds inline retry/basic-backup handling for failed or empty-array step generation and corrects current Finish completion so it sends no hidden rating or pantry-update claim. The full Phase 4 revamp must also reject blank/non-cookable generated instructions before entering or saving Live Cooking state. Full Ready Check, Coach Feed, timer redesign, provider prompts, schema, and Phase 5 cleanup remain planned |
...
| #236 | Ready for review | `codex/init-001-cooking-step-recovery` | Current Phase 4 slice: Live Cooking shows inline recovery with Retry/basic-backup choices when cooking-step generation fails or returns an empty step array, exposes final-step `Finish`, and omits hidden rating/notes plus pantry-update copy on linked completion. Focused unit coverage is in `tests/unit/live-cooking-guest-session.test.tsx`; blank/non-cookable generated-step validation is now documented as required for the full Phase 4 revamp. No Ready Check, Coach Feed, timer redesign, provider prompt, schema, or Phase 5 cleanup changes |
...
### 2026-06-25 - Phase 4 cooking-step recovery branch opened

Codex opened PR #236 (`codex/init-001-cooking-step-recovery`) from fresh `origin/main` `0c94f76` after triage skipped open/owned PRs #232, #234, and #235. The branch is a narrow Phase 4 runtime slice for honest Live Cooking recovery and Finish semantics: failed or empty cooking-step generation now shows an inline recovery panel with `Try again`, explicit `Use basic steps`, and `Back to Planning`; the final step exposes `Finish`; and linked completion omits hidden rating/notes while using the accepted cooking-history/cleanup copy. The branch intentionally leaves Ready Check, Coach Feed, timer redesign, provider prompt changes, schema, and Phase 5 cleanup state out of scope.

Focused local coverage in `tests/unit/live-cooking-guest-session.test.tsx` protects the value claim that cooks are not silently moved into generic instructions after a provider failure and that completed linked cooks do not receive invented rating or pantry-update semantics. Human Replit validation is not required before merge for this narrow client/state correction if exact-head GitHub `unit` and `e2e_guest_smoke` pass, but Replit/mobile speech and persistence smoke remains useful before broader Phase 4 closeout or release-batch validation.
