...
**Active PR:** None yet for the Phase 3.1 recipe preview imagery implementation.
...
The active Phase 3.1 recipe preview imagery implementation is in progress on `codex/init-001-recipe-preview-images` as of 2026-06-16. The branch adds an async/cache-backed recipe image resolver, the `recipe_image_cache` schema metadata, Replit App Storage object storage, strict title/core-ingredient cache matching, an image accuracy judge before approval, all-or-none client hydration into the existing Ticket Pass/Prep Tray `imageUrl` slots, and a curated fixture seed path for pre-generated library entries. Runtime generation is explicitly gated behind `RECIPE_IMAGE_GENERATION_ENABLED=true`; cache hits can still serve while generation is disabled. EFF-022 is related but unchanged because this branch does not alter recipe prompt cuisine behavior.
...
| Phase 3.1 | In progress | [#69](https://github.com/wmishak404/laica/pull/69), [#73](https://github.com/wmishak404/laica/pull/73), [#75](https://github.com/wmishak404/laica/pull/75), [#81](https://github.com/wmishak404/laica/pull/81), [#141](https://github.com/wmishak404/laica/pull/141), [#175](https://github.com/wmishak404/laica/pull/175), [#184](https://github.com/wmishak404/laica/pull/184); [#78](https://github.com/wmishak404/laica/pull/78) closed unmerged; active branch `codex/init-001-recipe-preview-images` | Kickoff/audit, Planning copy/count emphasis, Slop Bowl pantry-check chip alignment, Setup/Settings inventory review chip states, Slop Bowl generated-result button typography alignment, PR #78 abandonment/retry brief, accepted Ticket Pass hierarchy retry, and Planning toast cleanup are merged. Recipe preview imagery is implemented locally on the active branch with Replit/App Storage live validation still required before enabling or merging. Remaining Phase 3.1 scope after imagery is light Prep Tray alignment if needed, ingredient chip unification, and closeout visual review |
...
| #192 | Open | `codex/init-001-recipe-preview-images` | Phase 3.1 async recipe preview imagery pipeline; local checks passed and GitHub `unit`, `e2e_guest_smoke`, `npm-audit`, `trufflehog_pr`, and CodeQL passed at head `ab6b951`; Replit schema/App Storage/live image smoke remains pending before merge readiness |
...
1. Finish the active Phase 3.1 recipe preview imagery branch `codex/init-001-recipe-preview-images` before enabling runtime image generation: apply the `recipe_image_cache` schema through the Replit-authoritative path, confirm Replit App Storage, run one real three-recipe smoke, inspect title/core-ingredient accuracy, confirm Ticket Pass and Prep Tray image loading, and record model, quality, latency, cost, SHA, and negative scope. After imagery, the next documented Phase 3.1 candidates are light Prep Tray shell alignment if needed, ingredient chip unification, or closeout visual review.
