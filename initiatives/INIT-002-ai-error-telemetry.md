...
|---|---|---|---|
| Phase 0 — INIT hub + PD-010 | Merged | [#41](https://github.com/wmishak404/laica/pull/41) (`cb94f28`) | EFFORT-019, INIT-002, PD-010, active-list updates landed on `main` 2026-05-08; EFFORT-019 later resolved into this INIT |
| Phase 1 — stdout logger + 9 routes | Planned (next) | TBD | EFFORT-018 merged via [#43](https://github.com/wmishak404/laica/pull/43). Phase 1 builds a server-side `classifyAiError` mirroring EFFORT-018's taxonomy, request-id middleware, and JSON stdout logger wired into 9 AI route catch blocks |
...
| Phase 3 — DB schema + writer | Planned | TBD | `ai_error_events` schema + bounded writer + Replit `db:push` per EFFORT-010 |
...
|---|---|---|---|
| [#41](https://github.com/wmishak404/laica/pull/41) | Merged | `claude/elated-poincare-269ddc` | Docs-only; squash-merged at `cb94f28` on 2026-05-08. Rebased twice during review: once onto `bc242a0` (PR #40 Replit Validation Focus Guide) and once onto `24decb2` (PR #44 EFFORT-018 closeout) |
...
| [EFFORT-019](../efforts/effort-019-ai-error-telemetry-and-eval-monitoring.md) | Resolved historical filing; active telemetry implementation now lives in this INIT |
...
- 2026-05-07 — During plan review, the pure error classifier was reassigned from INIT-002 Phase 0 to INIT-002 Phase 1 to resolve an ownership ambiguity with [EFFORT-018](../efforts/effort-018-authenticated-ai-error-handling.md). EFFORT-018 already owns the user-facing classification taxonomy; building the classifier inside it (or letting Phase 1 build it after EFFORT-018 merges) avoids duplicate work and merge collisions on the 9 AI route catch blocks.
...
- 2026-05-07 — EFFORT-018 ([PR #43](https://github.com/wmishak404/laica/pull/43)) merged with a *client-side* classifier in `rateLimitHandler.ts` / `queryClient.ts` and *typed server-side error payloads* in `server/routes.ts` — but no server-side classifier function. Phase 1 now owns building `server/aiErrorClassifier.ts` mirroring EFFORT-018's wider taxonomy (400/401/403/404/413/429/5xx/network) so the user-facing copy and telemetry stay aligned. This is a clarification of ownership, not a new phase.
...
1. `git fetch origin && git checkout -b claude/init-002-phase-1-stdout-logger origin/main`. Verify [`server/ai-privacy.ts`](../server/ai-privacy.ts) and EFFORT-018's typed-error payloads in [`server/routes.ts`](../server/routes.ts) are present (they should be, since both are on `main`).
2. Symlink `.env.keys` per [`CLAUDE.md`](../CLAUDE.md) worktree note if running locally.
3. Read [EFFORT-018's client-side classifier](../client/src/lib/rateLimitHandler.ts) and [`queryClient.ts`](../client/src/lib/queryClient.ts) to extract the canonical taxonomy (400/401/403/404/413/429/5xx/network).
4. Build:
...
If the v0 `error_class` enum in PD-010 cannot cleanly express EFFORT-018's wider HTTP taxonomy, propose an enum expansion as a PD-010 amendment in the same PR.

## Chronology

- **2026-05-07** — Wilson asked for parallel persistent-error-logging design while EFFORT-018 ships authenticated AI error UX. Claude reviewed EFFORT-019, ran exploration agents over the AI route surface, admin pattern, and sibling docs (EFFORT-018, EFFORT-010, mobile-refresh AI privacy), and produced a phased implementation plan.
- **2026-05-07** — Wilson approved the plan with four decisions: (1) stdout-only first then DB later, (2) wire all 9 AI routes in v1, (3) wait for EFFORT-018 to merge before Phase 1, (4) PD-010 at top-level. Asked to file the work as INIT-002 because it is now phased.
- **2026-05-07** — Phase 0 docs filed: EFFORT-019, INIT-002, PD-010, registry/README updates.
...
- **2026-05-07** — EFFORT-018 merged ([#43](https://github.com/wmishak404/laica/pull/43)) and was closed out ([#44](https://github.com/wmishak404/laica/pull/44)). Rebased Phase 0 docs branch onto `24decb2`. Renamed our EFFORT-019 file to match the canonical filename PR #44 created (`efforts/effort-019-ai-error-telemetry-and-eval-monitoring.md`). Flipped Phase 1 from `Blocked on EFFORT-018` to `Unblocked, planned`. Documented that EFFORT-018 ships a client-side classifier and typed server-side error payloads, but no server-side classifier function — INIT-002 Phase 1 owns that mirror.
...
- **2026-05-09** — Effort cleanup resolved EFFORT-019 as a standalone item. Active implementation remains in INIT-002, with PD-010 and `docs/workflows/ai-error-handling-and-telemetry.md` as the durable governance/workflow entrypoints.
