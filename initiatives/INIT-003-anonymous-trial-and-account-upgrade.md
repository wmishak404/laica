...
As of 2026-07-21, ready-for-review [PR #324](https://github.com/wmishak404/laica/pull/324) implements and validates the production-readiness correction for cooking completion. Guest Finish stays local and uses the honest prompt `Dinner's ready. Sign up to save this session to your cooking history.` Linked saved-History wording is withheld until the completion mutation succeeds; a failed linked mutation preserves the local recovery state and exposes `Try Finish again`. Transcript, speech, toast, and the persistent retry status now derive from the same typed outcome. Local checks, the full GitHub E2E lane, a direct linked persistence/History check, and guest Replit checks at app-reported `390x844` and `412x915` passed. The branch is rebased onto current `origin/main`; the PR is unmerged pending Wilson's explicit approval.
...
| Phase 4 — linked-account save boundary and promotion | First slice complete; completion truthfulness correction ready for review | [#126](https://github.com/wmishak404/laica/pull/126) / `codex/anonymous-google-promotion`; [#156](https://github.com/wmishak404/laica/pull/156) / `codex/remove-guest-bottom-nav-menu`; [#324](https://github.com/wmishak404/laica/pull/324) / `codex/guest-finish-persistence-outcome` | PR #126 merged as `8282d51`; preserves same-browser Pantry, Kitchen, Cooking Profile, and favorite chefs through Google conversion, uses neutral import consent for credential-in-use flows, shows inline success confirmation, and keeps durable History import deferred. PR #156 merged as `492b3a6`; removes the unapproved guest-only `Save progress` bottom-nav shortcut while keeping promotion available through the planning reminder and app menu. Ready-for-review PR #324 makes Finish claims conditional on the actual guest/local, linked-save-success, or linked-save-failure outcome and preserves retry state after linked persistence failure. |
...
| [#324](https://github.com/wmishak404/laica/pull/324) | Ready for review | `codex/guest-finish-persistence-outcome` | Final-base-refresh implementation `3719fe2c` uses one typed outcome across transcript, speech, toast, and retry status. Local check/build/audit, focused Live Cooking Vitest (46), full Vitest (50 files / 391 tests), and the full GitHub E2E lane pass. Replit at `8721683d` passed a linked persisted Finish/History check and honest guest Finish at app-reported `390x844` and `412x915`; exact latest-head workflow evidence stays in the live PR. |
...
- Ready-for-review PR #324 implements that outcome contract from the exact documented production-readiness base `08fa856d`, then rebases it onto current `origin/main` after the intervening docs and lockfile-audit work. Local unit evidence proves guest/local copy rejects saved-History claims, linked success produces the claim only after the deferred mutation resolves, linked failure retains the scoped cooking recovery record and can retry to success, and speech/toast/transcript/status copy remain identical for the selected outcome. The full GitHub E2E lane and both Replit mobile viewports passed; merge and production publish remain separate Wilson-authorized steps.

## Current Resume Point

Review draft PR #324 together with the separately owned EFF-033 PR before the next production publish. PR #324's implementation and required automated/mobile evidence now pass, but the 2026-07-17 candidate `2686117a` remains not release-ready and no publish is authorized by this work.
...
Ready-for-review PR #324 now implements that contract. Guest completion clears only the browser-local recovery record and announces sign-up before History; linked completion waits for the durable mutation before clearing recovery/calling the parent completion callback; linked failure keeps the recovery record and exposes an honest retry. The focused unit suite covers timing, retry, and cross-surface consistency; the full GitHub E2E lane passes; and Replit checks passed linked persistence/History plus honest guest completion at app-reported `390x844` and `412x915`. The PR remains unmerged pending Wilson's explicit approval.
