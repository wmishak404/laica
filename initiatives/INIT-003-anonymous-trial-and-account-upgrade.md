...
As of 2026-07-21, [PR #324](https://github.com/wmishak404/laica/pull/324) has squash-merged as `af36e8f03d8cdbb2d3c2178d2726eb8ea8e6bf6a` from final head `c34380abd786f65487ed2aad504f4415a336559a`, completing the production-readiness correction for cooking completion. Guest Finish stays local and uses the honest prompt `Dinner's ready. Sign up to save this session to your cooking history.` Linked saved-History wording is withheld until the completion mutation succeeds; a failed linked mutation preserves the local recovery state and exposes `Try Finish again`. Transcript, speech, toast, and the persistent retry status derive from the same typed outcome. Exact-head GitHub automation passed the combined nine-test guest + linked Playwright lane, unit, dependency audit, secret scan, and CodeQL. Direct-shell Replit validation at the same final head passed honest guest Finish at app-reported `390x844` and `412x915` without Replit Agent; the retained linked runtime evidence and exact-head schema-backed lane cover successful persistence/History, while deterministic unit evidence covers linked failure/retry without inducing a live database failure.
...
| Phase 4 — linked-account save boundary and promotion | First slice and completion truthfulness correction complete | [#126](https://github.com/wmishak404/laica/pull/126) / `codex/anonymous-google-promotion`; [#156](https://github.com/wmishak404/laica/pull/156) / `codex/remove-guest-bottom-nav-menu`; [#324](https://github.com/wmishak404/laica/pull/324) / `codex/guest-finish-persistence-outcome` | PR #126 merged as `8282d51`; preserves same-browser Pantry, Kitchen, Cooking Profile, and favorite chefs through Google conversion, uses neutral import consent for credential-in-use flows, shows inline success confirmation, and keeps durable History import deferred. PR #156 merged as `492b3a6`; removes the unapproved guest-only `Save progress` bottom-nav shortcut while keeping promotion available through the planning reminder and app menu. PR #324 merged as `af36e8f` from final head `c34380a`; Finish claims now follow the actual guest-local, linked-save-success, or linked-save-failure outcome, and linked persistence failure retains retry state. |
...
| [#324](https://github.com/wmishak404/laica/pull/324) | Merged | `codex/guest-finish-persistence-outcome` | Squash-merged as `af36e8f03d8cdbb2d3c2178d2726eb8ea8e6bf6a` from final head `c34380abd786f65487ed2aad504f4415a336559a`. Exact-head GitHub run `29869916652` passed unit plus all nine guest/linked Playwright tests; dependency audit `29869916626`, secret scan `29869916744`, and CodeQL `29869913857` passed. Direct-shell Replit at the same head passed honest guest Finish at app-reported `390x844` and `412x915`; earlier linked runtime evidence passed persisted Finish/History, and deterministic unit coverage retains the non-destructive failure/retry proof. |
...
- The 2026-07-17 full `main` production-readiness run at `2686117a` revalidated real anonymous entry, same-browser guest setup/cooking restore, linked-only Slop Bowl and History gates, and guest -> existing Google promotion in Replit. The visible guest Finish toast correctly required sign-up before History, but the transcript/speech falsely claimed `Saved to your cooking history`; this contradicts the accepted linked-only durability model and blocks publish. Exact evidence and the recommended single-source completion-message fix are in [`docs/handoffs/2026-07-17-codex-production-readiness-main.md`](../docs/handoffs/2026-07-17-codex-production-readiness-main.md).
- Wilson's 2026-07-20 review requested the concrete fix recommendation. Remove the unconditional linked message from `nextStep()`, let completion produce a guest-local / linked-save-success / linked-save-failure outcome, and derive transcript, speech, and toast from one message for that outcome. Linked saved-History wording must wait for `completeSessionMutation` success; guest and linked-failure paths must explicitly reject that claim. Details and screenshot/Effort routing are in [`docs/handoffs/2026-07-20-codex-production-readiness-effort-routing.md`](../docs/handoffs/2026-07-20-codex-production-readiness-effort-routing.md).
- PR #324 implemented that outcome contract from the exact documented production-readiness base `08fa856d`, completed its accumulated-main rebase, and merged as `af36e8f` from final head `c34380a`. Local unit evidence proves guest/local copy rejects saved-History claims, linked success produces the claim only after the deferred mutation resolves, linked failure retains the scoped cooking recovery record and can retry to success, and speech/toast/transcript/status copy remain identical for the selected outcome. Exact-head schema-backed automation and the required direct-shell Replit guest mobile refresh passed. No production publish occurred.
- The 2026-07-22 full rerun at current `main` `742694d9` independently repeated a real guest provider cook and Finish at app-reported `390x844` and `412x915`. Transcript/toast used `Dinner's ready. Sign up to save this session to your cooking history.` and contained no false saved-History claim. Guest local persistence and linked-only Slop Bowl/History boundaries passed. After browser control recovered, real Google upgrade loaded Wilson's linked profile; linked Pantry/Tools reload persistence, Slop Bowl, Ticket Pass reload restore, a 14-step cook, linked Finish, and History write/cleanup all passed. Linked transcript/toast used `Dinner's ready. Saved to your cooking history. Pantry cleanup comes next.` only after persistence, and the History row was verified before deletion. No production publish occurred.

## Current Resume Point

Treat PR #324's completion contract as merged on `main` through `af36e8f`. The 2026-07-22 combined candidate `742694d9` freshly passes guest and linked Finish truthfulness plus the wider guest/provider/real-Google matrix, while the 2026-07-17 candidate `2686117a` remains historical failed readiness evidence. Wilson accepts the candidate as ready for production and owner-confirmed audible TTS works on `main`; spoken microphone input remains separate. No production publish occurred in the validation task, and admin valid/throttle remains a mandatory post-deploy smoke. INIT-003 implementation planning otherwise resumes at Phase 5/later promotion after INIT-001 Phase 5 has merged semantics; viewport and Settings/timer follow-up ownership remains linked from INIT-001.
...
5. Treat the Guest Finish completion contract as merged through PR #324 (`af36e8f`): `guest-local` completes only in the browser, `linked-saved` is the only outcome allowed to claim durable History success, and `linked-save-failed` stays in Live Cooking with retry state.
6. Use conversion-gated durability for guest promotion: first preserve Pantry, Kitchen, and Cooking Profile when the user links Google and consents; do not bulk- or background-import every completed anonymous cook into durable History in the first promotion slice.
7. Keep durable History, cleanup memory, taste memory, next-meal retention, and durable cooking-session memory linked-account only unless a later INIT-003 phase changes that boundary. A future Phase 5/promotion path may explicitly save a current guest cook after conversion, but that must be user-consented and designed with cleanup/taste semantics instead of automatic retro-import.
...
### 2026-07-17 — Production-readiness run found guest Finish honesty regression

The exact-`main` Replit run completed a full guest setup, provider-backed recipe, 11-step cooking guide, and Finish. Guest persistence and the linked-only Slop Bowl/History gates behaved correctly, and promotion into an existing Google session preserved the kitchen as designed. At Finish, however, the visible toast said `Sign up before saving cooking history` while transcript/speech said `Saved to your cooking history. Pantry cleanup comes next.` No guest History save occurred.

The split message comes from an unconditional linked completion transcript set before the guest-aware completion branch. Fix this by deriving one mode-aware completion message for speech, transcript, and toast, then reverse the existing guest unit assertion that currently codifies the false linked claim. Treat this as a production blocker because it breaks the initiative's central durability boundary even though the underlying save authorization remains correct.

### 2026-07-20 — Completion fix recommendation recorded

Wilson asked for the recommended correction before implementation. The recommendation is outcome-driven rather than a copy-only conditional: remove the unconditional linked response from `nextStep()`, have completion distinguish guest-local completion, linked persistence success, and linked persistence failure, and then use one canonical message for transcript, speech, and toast. Only linked persistence success may claim the cook was saved to History. Guest coverage should reject that claim, linked success coverage should prove it appears only after mutation success, and linked failure coverage should preserve an honest retry/recovery path.

PR #324 implements that contract. Guest completion clears only the browser-local recovery record and announces sign-up before History; linked completion waits for the durable mutation before clearing recovery/calling the parent completion callback; linked failure keeps the recovery record and exposes an honest retry.

### 2026-07-21 — Guest Finish outcome merged via PR #324
...
### 2026-07-22 — Combined-candidate guest and linked Finish rerun passed

Current `main` `742694d9` received a fresh full guest/provider/mobile regression instead of inheriting PR #324's result. A real eight-step guest cook completed at app-reported `390x844` and `412x915`; transcript and toast shared the honest sign-up-before-History message with no saved-History claim. Exact-head GitHub automation passed all nine guest + linked dev-auth Playwright tests, unit/coverage, audit, secret scan, CodeQL, and OAuth-start preflight.

The initially interrupted signed-in browser was recovered. Real Google upgrade loaded the existing linked profile; linked Pantry and Tools persisted across reload and cleanup; Slop Bowl and Ticket Pass reload restore passed; and a 14-step linked cook completed. Saved-History wording appeared only after persistence, the new cook appeared at the top of History, and the synthetic row was removed. A duplicate linked local-file scan was blocked by Chrome extension file-URL permission, but the exact preview's real image upload/vision route had already passed in the guest browser. The remaining release gap is outside INIT-003's completion contract: masked valid/throttled admin probes need the Replit-secret shell path.
