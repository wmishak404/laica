...
Treat PR #324's completion contract as merged on `main` through `af36e8f`. The 2026-07-17 candidate `2686117a` remains historical failed readiness evidence; no production publish occurred. The next release step is to select the then-current `main` SHA and run the production-validation registry's baseline plus changed-since-last-production checks before any separately authorized publish. INIT-003 implementation planning resumes at Phase 5/later promotion after INIT-001 Phase 5 has merged semantics; EFF-032 and EFF-034 remain deferred, non-blocking work.
