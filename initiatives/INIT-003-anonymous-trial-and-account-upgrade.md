...
| Phase 4 — linked-account save boundary and promotion | First slice complete; completion truthfulness correction in review | [#126](https://github.com/wmishak404/laica/pull/126) / `codex/anonymous-google-promotion`; [#156](https://github.com/wmishak404/laica/pull/156) / `codex/remove-guest-bottom-nav-menu`; [#324](https://github.com/wmishak404/laica/pull/324) / `codex/guest-finish-persistence-outcome` | PR #126 merged as `8282d51`; preserves same-browser Pantry, Kitchen, Cooking Profile, and favorite chefs through Google conversion, uses neutral import consent for credential-in-use flows, shows inline success confirmation, and keeps durable History import deferred. PR #156 merged as `492b3a6`; removes the unapproved guest-only `Save progress` bottom-nav shortcut while keeping promotion available through the planning reminder and app menu. Draft PR #324 makes Finish claims conditional on the actual guest/local, linked-save-success, or linked-save-failure outcome and preserves retry state after linked persistence failure. |
...
| [#324](https://github.com/wmishak404/laica/pull/324) | Draft | `codex/guest-finish-persistence-outcome` | Rebased implementation `d71daecc` uses one typed outcome across transcript, speech, toast, and retry status. Local check/build/audit, focused Live Cooking Vitest (46), full Vitest (50 files / 391 tests), and the full GitHub E2E lane pass. Replit at `8721683d` passed a linked persisted Finish/History check and honest guest Finish at app-reported `390x844` and `412x915`; final-head workflow/resync evidence stays in the live PR. |
...
Draft PR #324 now implements that contract. Guest completion clears only the browser-local recovery record and announces sign-up before History; linked completion waits for the durable mutation before clearing recovery/calling the parent completion callback; linked failure keeps the recovery record and exposes an honest retry. The focused unit suite covers timing, retry, and cross-surface consistency; the full GitHub E2E lane passes; and Replit checks passed linked persistence/History plus honest guest completion at app-reported `390x844` and `412x915`. The PR remains draft and unmerged.
