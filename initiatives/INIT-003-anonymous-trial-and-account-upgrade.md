...
As of 2026-07-20, draft [PR #324](https://github.com/wmishak404/laica/pull/324) implements the production-readiness correction for cooking completion. Guest Finish stays local and uses the honest prompt `Dinner's ready. Sign up to save this session to your cooking history.` Linked saved-History wording is withheld until the completion mutation succeeds; a failed linked mutation preserves the local recovery state and exposes `Try Finish again`. Transcript, speech, toast, and the persistent retry status now derive from the same typed outcome. Exact-head CI/E2E and Replit mobile evidence remain required before the blocker can be treated as cleared.
...
| [#324](https://github.com/wmishak404/laica/pull/324) | Draft | `codex/guest-finish-persistence-outcome` | Initial runtime head `5d41870b` implements one typed completion outcome across transcript, speech, toast, and retry status. Local `npm run check`, `npm run build`, focused Live Cooking Vitest (46 tests), and full Vitest (50 files / 391 tests) pass. Exact-head GitHub E2E and Replit iPhone/Pixel validation remain pending. |
...
- Draft PR #324 implements that outcome contract from the exact documented production-readiness base `08fa856d`. Local unit evidence proves guest/local copy rejects saved-History claims, linked success produces the claim only after the deferred mutation resolves, linked failure retains the scoped cooking recovery record and can retry to success, and speech/toast/transcript/status copy remain identical for the selected outcome. The release blocker remains open until exact-head automated E2E and both required Replit mobile viewports pass.

## Current Resume Point

Complete review and validation of draft PR #324 before the next production publish. The implementation now makes every Finish channel follow the persistence outcome, but the blocker is not cleared until exact-head GitHub E2E and Replit iPhone-/Pixel-like runtime checks are recorded. The 2026-07-17 candidate `2686117a` remains not release-ready.
...
Draft PR #324 now implements that contract. Guest completion clears only the browser-local recovery record and announces sign-up before History; linked completion waits for the durable mutation before clearing recovery/calling the parent completion callback; linked failure keeps the recovery record and exposes an honest retry. The focused unit suite covers timing, retry, and cross-surface consistency, and the guest Playwright smoke now rejects saved-History wording. Exact-head GitHub and Replit mobile evidence remain the final review gates.
