...
- Wilson's 2026-07-07 Live Cooking step-preview label QA from PR #260, preserved as a redacted eval intake seed and implemented as the first synthetic `live_cooking_step_previews` fixture lane after Phase 4 peer review.
...
| TBD | In progress | `codex/init-004-step-preview-evals` | Live Cooking step-preview/action-label eval lane. Adds first-class `live_cooking_step_previews` feature/criteria, deterministic fixture validation, two synthetic public fixtures, and docs/handoff updates after INIT-001 Phase 4 peer review and the PR #260 merge baseline; no runtime prompts, PR #260 branch changes, provider runs, private fixtures, DB migration, user-facing UI, or pixel-fit checks. |
...
The Live Cooking step-preview label branch `codex/init-004-step-preview-evals` is local/offline eval-harness work. It registers Wilson's PR #260 QA examples as a redacted intake, adds first-class `live_cooking_step_previews` feature/criteria, extends public fixture validation, and commits two synthetic fixtures: one client-rescue positive guard and one rendered-fragment negative guard. It was rebased onto PR #260's merged baseline `72df557` and mirrors the runtime first-pass rendering limits of 5 words / 24 characters. It does not change runtime code, production prompts, PR #260 branch state, provider behavior, DB schema, user-facing UI, private fixtures, or pixel/visual fit checks. Replit validation is not required for this offline eval branch.
...
The latest in-progress INIT-004 branch is `codex/init-004-step-preview-evals`. It implements `live_cooking_step_previews` as a distinct eval family for Wilson's PR #260 Live Cooking step-preview/action-label failures after INIT-001 Phase 4 peer review accepted the boundary. PR #260 has since merged as `72df55749b8c9a83ad6e5d5123a64592eb40dbfb` from validated head `0040f9f43d78634b0341a20a16a43c3c5a06109d`; this eval branch is rebased on that baseline and ready for review as an offline eval-harness slice once final checks and the alignment handoff are pushed.

The next bounded Phase 3 candidates are:

1. Open or review the INIT-004 step-preview eval PR from `codex/init-004-step-preview-evals`; keep it separate from PR #260 runtime/prompt work.
2. Start narrow LLM-judge calibration only after Wilson labels enough `live_cooking_step_previews` fixtures to measure TPR/TNR.
3. Add more fixtures only when they cover a new accepted label gap, such as near-duplicate labels or pixel/visual card-fit evidence.
