...
The eval-summary Phase 3 harness slice is in progress on `codex/init-004-eval-summary`. It builds on PR #219's `pantry_recipes` eval-surface and prompt-version provenance by making `/api/admin/eval/pending` report eligible versus skipped pending rows and making `/api/admin/eval/summary` include feature-level and prompt-version-level aggregates. This is admin/reporting plumbing only; it does not submit provider judge batches, process new eval results, change prompts, ingest private fixtures, change schema, expose user UI, activate daily reports, or decide EFF-022 cuisine fallback behavior.
...
| Phase 3 - Eval harness | In progress | [#188](https://github.com/wmishak404/laica/pull/188) merged / [#190](https://github.com/wmishak404/laica/pull/190) merged / [#198](https://github.com/wmishak404/laica/pull/198) merged / [#200](https://github.com/wmishak404/laica/pull/200) merged / [#205](https://github.com/wmishak404/laica/pull/205) merged / [#219](https://github.com/wmishak404/laica/pull/219) merged / `codex/init-004-eval-summary` active | Active slice adds eval queue/reporting summaries over separated eval surfaces and prompt-version provenance; provider judges, private fixtures, DB migrations, prompt activation, daily reports, and EFF-022 product-rule changes remain out of scope |
...
| TBD | Active branch | `codex/init-004-eval-summary` | Eval pending/summary admin reporting slice. Local focused evaluator/admin tests passed before broader validation; no Replit validation expected because no provider, prompt, schema, deployment, UI, private fixture, or user-facing runtime behavior changes. |
...
The eval-summary Phase 3 slice is local/offline admin/reporting plumbing. It makes pending eval queue eligibility visible before batch submission and groups completed eval results by eval surface and prompt version, so future eval reports can distinguish `pantry_recipes` rows that share the `recipe_suggestions` prompt path. It does not call providers, start judge batches, process new eval results, write schema, change prompts, ingest private fixtures, expose user UI, activate daily reports, or decide EFF-022 cuisine fallback behavior.
...
- **2026-06-23** - Started `codex/init-004-eval-summary` from fresh `origin/main` `89ce14f` after triage found open PR #220 was active Effort work and no active INIT-004 PR existed. The bounded milestone adds pending queue eligibility summaries and completed eval feature/prompt-version aggregates without provider judges, private fixtures, prompt changes, DB/schema changes, user-facing UI, daily reports, or EFF-022 cuisine fallback decisions.
