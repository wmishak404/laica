...
| TBD | In progress | `codex/init-004-step-preview-evals` | Live Cooking step-preview/action-label eval lane. Adds first-class `live_cooking_step_previews` feature/criteria, deterministic fixture validation, two synthetic public fixtures, and docs/handoff updates after INIT-001 Phase 4 peer review; no runtime prompts, PR #260 branch changes, provider runs, private fixtures, DB migration, user-facing UI, or pixel-fit checks. |
...
The latest in-progress INIT-004 branch is `codex/init-004-step-preview-evals`. It implements `live_cooking_step_previews` as a distinct eval family for Wilson's PR #260 Live Cooking step-preview/action-label failures after INIT-001 Phase 4 peer review accepted the boundary. The branch is ready for review as an offline eval-harness slice once final checks and the Phase 4 handoff are pushed.

The next bounded Phase 3 candidates are:

1. Have the INIT-001 Phase 4 PR #260 thread review the pushed handoff for whether the fixture shape reveals any prompt/fallback issue before PR #260 merge.
