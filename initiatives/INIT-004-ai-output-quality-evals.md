...
The report-export Phase 3 slice merged in [PR #246](https://github.com/wmishak404/laica/pull/246) as `690fe2c` on 2026-07-08T00:21:46Z. It builds on PR #232 by adding a protected `GET /api/admin/eval/report?format=json|markdown` artifact endpoint. The artifact keeps eval run rows, criterion aggregates, Judge Metrics, and Provider Input Inventory portable for PRs/handoffs/release notes while omitting raw request and model-response payloads. This is admin/reporting plumbing only; it does not submit provider judge batches, process new eval results, change prompts, ingest private fixtures, change schema, expose user UI, activate daily reports, or decide EFF-022 cuisine fallback behavior.
...
Wilson's 2026-06-30 EFF-022 direction accepts transparent pantry fallback as the preferred product behavior when a selected cuisine is weakly supported by the user's pantry. Missing cuisine staples may be explored as an optional helper, but Laica should not silently replace the selected cuisine or default the user to `No preference`. Runtime prompt/UI work remains deferred until after higher-priority INIT-001 work, and EFF-022 still owns the exact activation threshold and copy before cuisine-fit fixtures or prompt candidates claim that fallback behavior is implemented.
...
Wilson's 2026-07-07 PR #260 Live Cooking QA surfaced step-preview/action-label failures such as `Bring 4 Cups`, `Heat Oil Butter`, `Push Vegetables Side`, `Add Cold Cooked`, and repeated generic labels for different fried-rice milestones. Wilson's same-day Replit observation added a grammar-agreement failure: `Prep Leek` should render as `Prep Leeks` when the accepted recipe step prepares multiple leeks. The seed record [live-cooking-step-preview-label-seed-2026-07-07](../docs/evals/intakes/live-cooking-step-preview-label-seed-2026-07-07.md) and first public fixture batch establish a distinct `live_cooking_step_previews` eval family because the artifact is the small Live Cooking recall card, not recipe-generation quality and not broad `cooking_steps` safety/sequence quality. Phase 4 peer review accepted the family name and fixture fields; the first implementation adds deterministic schema/word-count/measurement/duplicate checks, seven synthetic provider-versus-rendered fixtures, and an opt-in uncalibrated judge-smoke runner. After PR #260 merged as `72df557`, the branch was rebased onto that baseline and the fixture rendering constraints were aligned to the runtime limits of 5 words and 24 characters. Runtime prompt changes, PR #260 fallback validation, pixel/visual fit checks, private fixtures, provider runs by default, and calibrated judges remain out of scope.
...
- Wilson's 2026-07-07 Live Cooking step-preview label QA from PR #260, plus the same-day Replit `Prep Leek` / `Prep Leeks` grammar observation, preserved as redacted eval intake signal and implemented as the first synthetic `live_cooking_step_previews` fixture lane after Phase 4 peer review.
...
Wilson's 2026-07-07 PR #260 QA examples are registered as [live-cooking-step-preview-label-seed-2026-07-07](../docs/evals/intakes/live-cooking-step-preview-label-seed-2026-07-07.md). Phase 4 peer review accepted `live_cooking_step_previews` as a distinct eval family for the small Live Cooking preview/action label. The user value is fast hands-busy recall: labels should usually be 2-4 words, stretch to 5 only when needed for meaning, avoid measurements, fit the preview card, avoid duplicate labels for distinct milestones in the same recipe, read as plain English with correct object grammar such as singular/plural agreement, and name the real cooking milestone. The merged PR #260 runtime baseline uses `STEP_ACTION_LABEL_MAX_WORDS = 5` and `STEP_ACTION_LABEL_MAX_CHARS = 24`; V1 fixtures mirror those limits.

Keep this lane separate from recipe-generation quality and from broad `cooking_steps` safety, equipment, sequence, and sensory-cue checks. The first implementation uses synthetic/redacted fixtures only: deterministic checks cover output shape, final rendered-label word/character limits, measurements, and exact sibling duplicates; criterion labels preserve provider label quality, rendered label quality, milestone fit, and plain-English quality for future human review. The current fixture batch includes focused failures for measurement fragments, wrong milestones, incomplete phrases, duplicate labels, and singular/plural agreement. Narrow LLM judge criteria and the optional judge-smoke runner exist only as uncalibrated triage until human labels and TPR/TNR are measured.
...
| Phase 3 - Eval harness | In progress | [#188](https://github.com/wmishak404/laica/pull/188) merged / [#190](https://github.com/wmishak404/laica/pull/190) merged / [#198](https://github.com/wmishak404/laica/pull/198) merged / [#200](https://github.com/wmishak404/laica/pull/200) merged / [#205](https://github.com/wmishak404/laica/pull/205) merged / [#219](https://github.com/wmishak404/laica/pull/219) merged / [#232](https://github.com/wmishak404/laica/pull/232) merged / [#246](https://github.com/wmishak404/laica/pull/246) merged / [#263](https://github.com/wmishak404/laica/pull/263) in progress | Latest merged slice added a protected redacted report artifact/export path over existing eval summary fields. Current in-progress branch adds first-class `live_cooking_step_previews` fixture validation, seven synthetic public fixtures aligned to PR #260's 5-word / 24-character runtime limits, and an opt-in uncalibrated judge-smoke runner; provider runs by default, private fixtures, DB migrations, prompt activation, daily reports, PR #260 runtime fallback validation, pixel/visual card fit, and runtime EFF-022 fallback changes remain out of scope |
...
| [#263](https://github.com/wmishak404/laica/pull/263) | In progress | `codex/init-004-step-preview-evals` | Live Cooking step-preview/action-label eval lane. Adds first-class `live_cooking_step_previews` feature/criteria, deterministic fixture validation, seven synthetic public fixtures including Wilson's `Prep Leek` / `Prep Leeks` grammar-agreement observation, and an opt-in uncalibrated judge-smoke runner after INIT-001 Phase 4 peer review and the PR #260 merge baseline; no runtime prompts, PR #260 branch changes, provider runs by default, private fixtures, DB migration, user-facing UI, or pixel-fit checks. |
...
PR #247 is docs-only product-direction capture for EFF-022, merged as `203e621` after Wilson's merge instruction. Final PR head `cd4bbca` passed local `git diff --check` plus GitHub `unit`, `e2e_guest_smoke`, `npm-audit`, `trufflehog_pr`, CodeQL, and CodeQL action / JavaScript-TypeScript analyses. No Replit validation was required because no runtime, schema, prompt, provider, UI, fixture data, deployment, or eval-run behavior changed.
...
The Live Cooking step-preview label branch `codex/init-004-step-preview-evals` is local/offline eval-harness work. It registers Wilson's PR #260 QA examples and the `Prep Leek` / `Prep Leeks` Replit grammar observation as redacted intake signal, adds first-class `live_cooking_step_previews` feature/criteria, extends public fixture validation, and commits seven synthetic fixtures: one client-rescue positive guard, one rendered-fragment negative guard, and five focused judge-smoke cases for measurement fragments, wrong milestones, incomplete phrases, duplicate labels, and singular/plural agreement. It was rebased onto PR #260's merged baseline `72df557` and mirrors the runtime first-pass rendering limits of 5 words / 24 characters. It also adds an opt-in `npm run eval:step-preview-judge-smoke` command that calls a provider only when explicitly run with local secrets; the checked-in tests mock this lane and do not call providers. It does not change runtime code, production prompts, PR #260 branch state, provider behavior by default, DB schema, user-facing UI, private fixtures, or pixel/visual fit checks. Replit validation is not required for this offline eval branch.
...
The latest merged Phase 3 implementation slice is PR #246, merged as `690fe2c`; it added a protected redacted eval report artifact/export path with row-first Markdown, source/run labeling, criterion aggregates, Judge Metrics, and Provider Input Inventory. It builds on [PR #232](https://github.com/wmishak404/laica/pull/232), merged as `5b8e7ed`, which added pending queue eligibility summaries, completed eval feature/prompt-version aggregates, removed weak top-level aggregate quality rates, and made the active recipe-generation eval taxonomy clearer with `chef_it_up_suggestions` and `slop_bowl_suggestions` canonical report keys plus legacy alias normalization.
...
The latest in-progress INIT-004 branch is [PR #263](https://github.com/wmishak404/laica/pull/263) on `codex/init-004-step-preview-evals`. It implements `live_cooking_step_previews` as a distinct eval family for Wilson's PR #260 Live Cooking step-preview/action-label failures after INIT-001 Phase 4 peer review accepted the boundary. PR #260 has since merged as `72df55749b8c9a83ad6e5d5123a64592eb40dbfb` from validated head `0040f9f43d78634b0341a20a16a43c3c5a06109d`; this eval branch is rebased on that baseline and now includes seven public synthetic fixtures plus an opt-in uncalibrated judge-smoke runner for Wilson review.

The next bounded Phase 3 candidates are:

1. Review PR #263 as the offline `live_cooking_step_previews` fixture and smoke-runner lane; keep it separate from PR #260 runtime/prompt work.
2. Optionally run `npm run env:run -- npm run eval:step-preview-judge-smoke -- --runs 3 --out /tmp/laica-step-preview-judge-smoke.md` and have Wilson inspect the uncalibrated report before treating any judge behavior as trustworthy.
3. Start narrow LLM-judge calibration only after Wilson labels enough `live_cooking_step_previews` fixtures to measure TPR/TNR.
4. Add more fixtures only when they cover a new accepted label gap, such as near-duplicate labels or pixel/visual card-fit evidence.
...
- **2026-06-30** - Wilson accepted the EFF-022 product direction for cuisine-constrained pantry results: use transparent pantry fallback by default when pantry support is weak, optionally ask for missing staples first, and avoid defaulting users to `No preference`. Implementation remains an enhancement after higher-priority INIT-001 work; EFF-022 owns the activation threshold and copy before INIT-004 labels or prompt candidates treat fallback behavior as implemented.
- **2026-06-30** - PR #247 merged the EFF-022 fallback decision record as `203e621` after Wilson's merge instruction and final-head local/GitHub checks passed. The merge is docs-only and does not change runtime prompts, UI, provider behavior, schema, fixture data, eval execution, or the deferred fallback activation threshold.
...
- **2026-07-07** - INIT-001 Phase 4 peer review accepted `live_cooking_step_previews` as the distinct family name and confirmed fixture fields should capture provider and rendered/runtime behavior. `codex/init-004-step-preview-evals` then added first-class eval feature/criteria support, deterministic fixture checks, two public synthetic fixtures, and updated registry/README/workflow/spec docs while leaving PR #260 runtime code and prompts untouched.
- **2026-07-07** - After PR #260 merged as `72df55749b8c9a83ad6e5d5123a64592eb40dbfb` from validated head `0040f9f43d78634b0341a20a16a43c3c5a06109d`, Phase 4 confirmed the fixture shape maps cleanly to provider `actionLabel`, `normalizeStepActionLabel(...)`, `deriveStepActionLabel(...)`, `buildStepPreviewLabels(...)`, and `getStepHeadline(...)`. The branch rebased onto the merged baseline and corrected fixture `renderingConstraints.maxCharacters` from the earlier synthetic `28` to runtime `24`.
- **2026-07-07** - Wilson shared a Replit Live Cooking run where a multi-leek prep step rendered `Prep Leek`; the eval branch captured this as a redacted synthetic `singular-plural-agreement` fixture expecting `Prep Leeks`, expanded the focused fixture batch to seven public fixtures, and added an opt-in uncalibrated judge-smoke runner for three-to-five-run review without committing screenshots or private traces.
