...
| [INIT-004](INIT-004-ai-output-quality-evals.md) | AI Output Quality Evals & Prompt Improvement | `In Progress` | Wilson / Codex / Claude / Replit | 2026-06-09 | Phase 3 - eval harness | `codex/init-004-eval-provenance` pending PR | Active branch adds criteria-aware eval queue selection and logs `/api/recipes/pantry` outputs as `pantry_recipes` with reused `recipe_suggestions` prompt-version provenance; no prompt, provider, schema, UI, private-fixture, daily-report, or EFF-022 product-rule change. |
