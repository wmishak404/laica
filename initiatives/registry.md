...
|---|---|---|---|---|---|---|---|
| [INIT-001](INIT-001-mobile-refresh.md) | Mobile Refresh | `In Progress` | Wilson / Codex / Claude / Replit | 2026-04-29 | Phase 3.1 follow-up / Phase 4 speech arbitration slice | [#191](https://github.com/wmishak404/laica/pull/191) ready-for-review cycle | PR #201 merged as `d37a4df`: Chef It Up / MealPlanning reload restoration now recovers valid active Ticket Pass and Prep Tray sessions within 15 minutes while explicit Back suppresses stale restore. PR #192 imagery behavior remains accepted: Ticket Pass placeholder-only, Prep Tray selected image, OpenAI default, and Gemini benchmarking deferred before any provider-default change. PR #191 now implements the speech-action matrix for initial Step 1 audio, interruption, Ask for Help, mute persistence, unmute-no-autoplay, transcript fidelity, rapid actions, timer interruption, and exit cleanup with focused deterministic coverage. PR #203 merged the unrelated landing auth-control contrast fix as `b6ba180`; the default local dotenvx DB drift remains an EFF-010 evidence-environment issue, so PR #191's remaining merge-evidence lane is exact-head GitHub `unit` / `e2e_guest_smoke` after rebase and Wilson's merge decision. |
...
| [INIT-004](INIT-004-ai-output-quality-evals.md) | AI Output Quality Evals & Prompt Improvement | `In Progress` | Wilson / Codex / Claude / Replit | 2026-06-09 | Phase 3 - eval harness | None | PR #200 merged as `bb5604f`: public fixture validation now has `npm run eval:fixtures` as a focused evidence lane. Next Phase 3 candidates are the next small target-set batch, `pantry_recipes` provenance work, and later calibrated narrow judges. |
