...
|---|---|---|---|---|---|---|
| [INIT-001](INIT-001-mobile-refresh.md) | Mobile Refresh | `In Progress` | Wilson / Codex / Claude / Replit | 2026-04-29 | Phase 4 follow-up / Phase 3.1 follow-up | 2026-07-07: `codex/init-001-phase4-timer-polish` opened after [#264](https://github.com/wmishak404/laica/pull/264) / closeout to make Live Cooking timers explicit-start, visible by default, opt-out beside CC, reset on step navigation, minimizable when active, and based on the current recipe step's real duration in `H:MM:SS` format with circular play/pause and reset buttons instead of separate provider-duration suggestion cards. Durationless steps do not invent a fallback timer. Wilson's Ready Check follow-up is folded in: larger Prep Tray loading copy plus rejected-image fallback, Back to Planning restores Prep Tray/session, and existing bottom nav remains visible on Ready Check while active cooking hides it. Focused Live Cooking/MealPlanning/app-shell Vitest, check, and build pass locally; exact-head GitHub checks and release/batch Replit validation remain pending/deferred. Full provider schema shape and Phase 5 cleanup remain planned. |
