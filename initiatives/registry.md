...
|---|---|---|---|---|---|---|
| [INIT-001](INIT-001-mobile-refresh.md) | Mobile Refresh | `In Progress` | Wilson / Codex / Claude / Replit | 2026-04-29 | Phase 4 follow-up / Phase 3.1 follow-up | 2026-07-13: `codex/init-001-cooking-step-schema` is the current bounded provider-schema foundation after PR #275. It normalizes cooking-step provider output before eval logging and route response and preserves `actionLabel` through linked cooking-session snapshots. It does not start Phase 5 cleanup, DB schema work, UI changes, provider prompt changes, live-provider runs, or the richer `suggestedTimer` object. |
