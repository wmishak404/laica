...
- Registry updated: 2026-07-22 by the full regression rerun branch.
- Current candidate: `742694d9d209dba04674ce7188319d7f449c4a6e`. Latest runtime merge is PR #324 at `af36e8f03d8cdbb2d3c2178d2726eb8ea8e6bf6a`; `742694d9` adds its fact-only closeout.
- Last broadly production-readiness-tested candidate: `742694d9d209dba04674ce7188319d7f449c4a6e`, with the explicit direct-live gaps below.
...
- Release verdict: **ready for production**. The recorded EFF-033 and Guest Finish corrections passed a fresh combined-candidate mobile rerun, the real-Google linked matrix completed on the exact Replit preview, and no new product blocker was found. Wilson subsequently confirmed audible TTS works on `main`, then confirmed camera access/capture and correct ingredient recognition from a small image set; spoken microphone input remains separately unclaimed. Valid-secret/admin throttling is a mandatory post-deploy smoke, the setup viewport limitation is an accepted release exception owned by EFF-035 immediately after production, and EFF-034 is post-release polish. Publishing still requires separate explicit authorization.

The 2026-07-22 rerun audited the accumulated first-parent merge history through `742694d9`, including the dependency/security-tool maintenance, linked Ticket Pass stability, E2E limiter capacity, EFF-033, and Guest Finish merges. Production remains unpublished.

## 2026-07-22 Full `main` Production-Readiness Rerun

- Detailed evidence and screenshots: [`docs/handoffs/2026-07-22-codex-production-readiness-rerun.md`](handoffs/2026-07-22-codex-production-readiness-rerun.md).
- Exact candidate: `742694d9d209dba04674ce7188319d7f449c4a6e`.
- Fresh pass: local/Replit install, check, build, 51 files / 399 unit tests, Replit DB health/startup, real provider recipes/steps/selected image, repository-image vision upload, Ready Check, eight-step Live Cooking, guest session/captions restore, feedback, toast gestures, guest Settings persistence, and security rejection paths.
- Fresh recent-fix pass: EFF-033 Pantry `390x844` and Tools `412x915` remained page-owned, opaque, flush to Cook/Menu, and clear/hit-correct at true scroll bottom; guest Finish at both sizes used the honest sign-up-before-History message with no false saved claim.
- Exact-head GitHub evidence: CI `29874990266` passed `9` guest + linked dev-auth Playwright tests in `51.8s`; audit `29874990223`, secret scan `29874990277`, CodeQL `29874990053`, and OAuth-start preflight `29921023021` passed.
- Accepted release exception reproduced: first-time Pantry/Tools actions remain unreachable with no scroll owner at `390x844`; at `375x667` the intended scroller has range and normal control interaction moves it, explaining browser/height variability. EFF-035 owns the generalized immediate post-production patch and supersedes EFF-032. EFF-034 Settings blank tail remains post-release polish. The provider guide had no timer, so the existing Reset-label finding is retained rather than called fixed.
- The controlled Codex run excluded camera permission/capture while image upload/vision passed. Wilson later owner-confirmed the real camera path works on `main` and ingredients were recognized correctly from a small image set; broader device/image-corpus coverage remains unclaimed.
- Fresh linked pass: real Google upgrade loaded the linked profile; Pantry/Tools save/reload and cleanup passed; Slop Bowl generated; the same three Ticket Pass suggestions restored after reload; a 14-step cook finished with saved-History wording only after persistence; and the new History row appeared and was removed. Chrome's extension blocked a duplicate linked local-file upload because file-URL access is disabled, while the same exact preview's real vision route had already passed in the guest browser.
- Validation ownership: missing/invalid admin `403` + no-cache and exact deterministic admin/rate-limit tests passed; use the real secret for mandatory valid/invalid/throttled/recovery smoke after deployment. Wilson owner-confirmed audible TTS plus bounded camera capture/ingredient recognition on `main`; one spoken microphone question remains a separate manual check. Safely induced provider failures remain resilience follow-up, not a release hold.
- No production publish occurred. The rerun found no new product defect. Wilson's follow-up decision created EFF-035 as the generalized immediate post-production viewport patch, superseded the narrower EFF-032 record without claiming implementation, and retained EFF-034 as post-release polish.
...
## 2026-07-21 PR #325 Merge Closeout
