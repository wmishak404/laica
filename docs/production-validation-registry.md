...
- Evidence so far: focused deterministic Vitest/CSS guards cover the timer state-machine behavior, Settings hub root classes, and hub fit CSS. Local full unit, typecheck/UI lint, production build, high/critical audit, and diff whitespace gates passed before PR creation. Exact-head GitHub `e2e_guest_smoke` and phone-viewport after evidence remain pending until the review head is finalized.
