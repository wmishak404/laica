...
- Playwright-managed dev servers explicitly bypass only the broad Vite/app-asset request limiter through `LAICA_E2E_APP_REQUEST_LIMIT_BYPASS=true`. The server rejects that bypass when `NODE_ENV=production`; `/api` and user/provider-specific limiters remain active. Do not set this flag on Replit or production runtime commands. Unexpected browser `429` responses are logged with the request/test context and attached under `test-results/`, which CI retains on E2E failure.
