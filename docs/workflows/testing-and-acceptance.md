...
For browser/UI bugs, collect the relevant route or screen, exact reproduction steps, observed and expected behavior, browser Console errors, and Network request **Response** bodies for the affected calls. Payload-only screenshots are not enough when the server can return terminal states such as `disabled`, `unavailable`, `pending`, `image_not_approved`, or provider/storage errors. Inspect DOM or client state when the bug is about a transient visual state, stale render, reload, cache, or auth-scoped data.

For Replit/runtime bugs, verify the running code and environment separately because git state, deployed runtime, secrets, database, and browser session can drift independently. Capture `git status --short --branch`, `git rev-parse HEAD`, and the relevant remote ref such as `git rev-parse origin/main` or the PR branch. Preserve local Replit commits on a side branch before any destructive sync. Capture server logs around the failing route, route status/response body, and relevant DB/cache rows or failure reasons when the feature uses persisted state.
