...
4. If schema work is involved, read EFFORT-010 and coordinate Replit-authoritative `db:push` instead of pushing casually from a local worktree.
...
Former EFFORT-019 is resolved as a standalone Effort. The work is not cancelled; it now lives in INIT-002, PD-010, and this workflow so agents do not split telemetry governance between a backlog file and the active initiative.
