...
| Closed-as-done confusion | An Effort was closed into an INIT even though the underlying work was not shipped, and the only remaining pointers lived in closed phase docs | If the work is unshipped but no longer standalone, add/update an explicit forward-looking follow-up section in the INIT with next likely phase/surface and resume trigger |
...
3. For each active Effort, ask:
   ...
   - If it moves under an INIT, where is the forward-looking resume point so future agents do not have to infer ownership from a closed phase?
...
4. Close or repoint stale Efforts in the same branch.
   ...
   - If the new home is an INIT, add or refresh a forward-looking follow-up section in that INIT. Do not rely only on historical deferrals inside closed phase records.
...
Wilson flagged an important ambiguity after the cleanup: closed standalone Efforts can read like shipped product work when their only remaining pointers are buried in already-merged phase docs.

The fix is now part of this workflow:
...
- If the work is still real but unshipped, also add a forward-looking follow-up section in the active INIT so future agents know where to resume it.
- Update the Effort closeout note and registry summary to point to that INIT section explicitly.
