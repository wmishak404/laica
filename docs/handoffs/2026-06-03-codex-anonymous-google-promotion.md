...
Wilson's Replit validation accepted the current product shape at runtime head `e2231be`: new Google sign-up and existing Google credential/import flows both looked good, Settings copy worked after removing repeated "this browser" language, and the neutral Google-import dialog was friendly enough for a brand-new-feeling account path. Follow-up Replit validation confirmed guest completed cooks do not retro-import into durable History after conversion and the guest recipe `#11` quota gate blocks as expected.

## Open items

- Replit validation should continue from the latest pushed commit. Wilson confirmed guest mental-model surfaces, same-browser setup persistence, new-account promotion, existing-credential import consent, linked profile writes, guest History non-import, and the quota `#11` gate. Remaining checks are the session-backed inline confirmation linger on latest head, the updated quota body copy (`Sign up before making more recipes.`), and a final branch-head refresh after subsequent commits. Popup cancel uses calm copy but may take about three seconds because Firebase reports popup closure asynchronously; avoid brittle focus-based cancellation unless Wilson later treats that delay as a blocker.
