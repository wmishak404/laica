...
- EFF-001: conforms to the no persistent app-header direction and keeps setup visual primitives scoped to setup utilities.
- EFF-004: implements the isolated `No restrictions` default-style choice.
- EFF-005: local checks passed, but Replit validation remains required before merge.
- EFF-012: implements the Replit visual review direction for setup chrome, camera controls, Kitchen accents, and illustration-style setup choices.
