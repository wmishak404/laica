...
- `initiatives/INIT-002-ai-error-telemetry.md`
  ...
  - Current Resume Point: rewrote as a numbered Phase 1 launch checklist with branch creation, EFFORT-018 source-of-truth files, the three new files to add, the 9 catch blocks to wire, the test plan, and the Replit Validation Focus Guide template to use in the PR.
...
No source code changed. No epic file modifications needed (EFFORT-019 is unchanged by the closeout; EFFORT-018 already `Resolved` on main).
...
2. EFFORT-018's wider HTTP taxonomy (400/401/403/404/413/429/5xx/network) is the source of truth for `error_class`. Read [`client/src/lib/rateLimitHandler.ts`](../../client/src/lib/rateLimitHandler.ts) and [`server/routes.ts`](../../server/routes.ts) for the exact shapes before writing the server-side classifier.
3. PD-010's v0 `error_class` enum (`validation | rate_limit | upstream_timeout | upstream_5xx | upstream_auth | unknown`) may not cleanly express EFFORT-018's wider taxonomy. If it doesn't, propose a PD-010 amendment in the same PR — that's the documented path.
...
- INIT-002 + PD-010 + EFFORT-019 cross-reference are on `main`.
- CLAUDE.md / AGENTS.md / `efforts/` / `initiatives/` indexes are current on `main`.
