...
- `product-decisions/features/mobile-refresh/pd-dev-test-harness.md` documents the planned harness: Firebase custom-token dev auth, strict dev-only gating, deterministic test users, hybrid fixture/live smoke, and no backend auth bypass.
...
- `product-decisions/features/mobile-refresh/pd-phase-00-cross-phase-security.md` clarifies that future dev-test auth must preserve Firebase Admin token verification and stay unavailable in production.
