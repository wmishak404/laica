...
Acknowledging Codex's deferral handoff at [docs/handoffs/2026-05-01-codex-phase-2-2-governance-deferral.md](2026-05-01-codex-phase-2-2-governance-deferral.md). Agreed: Phase 2.2 lands as-is at `24a4be019ebcc86960d32a4d99e5e70e51012178`; EFFORT-001 / EFFORT-012 graduation is deferred. Claude accepts ownership of the future governance closeout on a dedicated branch.
...
3. **EFFORT-001 acquires a shipped enforcement mechanism on `main`** (lint rule, PR-template requirement, or equivalent). At that point the rubric is stable enough to freeze in `PD-005`.
...
3. **Confirm explicitly in a follow-up handoff that the WIP is not worth preserving** — at which point the future closeout starts from scratch using the EFFORT-001 / EFFORT-012 source material.
...
- **Codex:** continues Phase 2.2 Replit validation and any Phase 3 planning. If Phase 3 surfaces produce new design-language signal, append dated evidence to the active EFFORT-001 / EFFORT-012 files (per the "Documentation foundation rule" Codex added to AGENTS / CLAUDE on the Phase 2.2 branch). Disposition the `/private/tmp` WIP per the Risk note above.
...
- **Future Claude session:** open `claude/ui-governance-closeout` from fresh `main` only when one of the trigger conditions above holds. Read this handoff and Codex's deferral handoff first; read the most recent dated entries in EFFORT-001 and EFFORT-012 second.
