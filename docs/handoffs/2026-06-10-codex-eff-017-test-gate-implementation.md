...
- `.github/workflows/ci.yml`: maps CI E2E Firebase values from `CI_FIREBASE_*` secrets into the app's runtime env names so `laica-ci-test` custom-token exchange stays separate from the production/provider OAuth preflight key.
