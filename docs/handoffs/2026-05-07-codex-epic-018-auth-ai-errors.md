# EPIC-018 authenticated AI error handling
...
Implemented EPIC-018 so authenticated AI failures no longer use demo-era copy or redirect users to `/`. The branch adds a shared typed request error, authenticated AI error classification, typed server payloads for AI/rate-limit failures, first-person plain-English copy, and tests for the status classes Wilson and Claude reviewed.
...
- `epics/018-authenticated-ai-error-handling.md`: updates EPIC-018 scope, decisions, copy principles, and telemetry deferral.
- `epics/019-ai-error-telemetry-and-eval-monitoring.md`: new separate epic for redacted operational AI error telemetry/evals.
- `epics/007-vision-scan-no-detection-feedback.md`, `epics/README.md`, `epics/registry.md`, and `initiatives/INIT-001-mobile-refresh.md`: update active epic/initiative context.
...
EPIC-018 intentionally does not implement persistent error logging. Use [EPIC-019](../../epics/019-ai-error-telemetry-and-eval-monitoring.md) for that parallel work, especially the allowlist-first redaction policy.

Phase 4 owns live-cooking-specific presentation: inline step retry/recovery, Coach Feed failure placement, and inline Feedback access. EPIC-018 only removes demo behavior and supplies shared classification/copy.
