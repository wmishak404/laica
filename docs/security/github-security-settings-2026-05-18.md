...
- `Secret Scan (TruffleHog)` is not required in the ruleset yet. It should be added only after the workflow cleanup proves green on pull requests.
...
- High static-analysis guardrail: CodeQL default setup is required on `main` through the GitHub code scanning results rule.
