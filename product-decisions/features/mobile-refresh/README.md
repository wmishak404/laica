...
This sequence is the default agenda, not a hard dependency graph. Phase 3.1 before Phase 4 is a soft sequence: Phase 3.1 is the next default design pass, but Phase 4 may start first if cooking guidance is the higher priority. For environment-parity / authenticated-smoke automation, see [EFF-017](../../../efforts/effort-017-environment-parity-and-ci-confidence.md) for current status and the accepted constraint that any pilot must not replace the current Replit validation gate.
