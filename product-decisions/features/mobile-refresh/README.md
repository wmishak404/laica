...
7. Continue Phase 3.1 from fresh `main` for the remaining design facelift plus async recipe imagery follow-ups. The Planning copy/count, Slop Bowl pantry-check chip alignment, Setup/Settings inventory review chip-state, Ticket Pass hierarchy, selected Prep Tray imagery, and Prep Tray ready-image fill slices have shipped; remaining work should preserve those decisions while tackling provider benchmarking before any provider-default change, ingredient chip unification, bottom nav fit if needed, and closeout visual review.
