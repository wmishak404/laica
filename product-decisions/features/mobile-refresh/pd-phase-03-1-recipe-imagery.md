...
Codex implemented the next narrow Planning-entry slice on `codex/mobile-refresh-phase-3-1-pantry-count-coral`, stacked from PR #71's `codex/mobile-refresh-phase-3-1-planning-copy` head. The status line now keeps the same sentence and empty-Pantry guard behavior, but highlights only the dynamic pantry-count phrase in Planning coral: `17 pantry items`, `1 pantry item`, or future count-label variants.

The implementation adds a small `getPlanningPantryCountLabel` helper so the full status sentence and highlighted phrase share pluralization logic, renders only the count phrase inside `.planning-pantry-count`, and keeps the surrounding helper sentence neutral. This slice intentionally did not touch broader Planning card/whitespace grammar, Slop Bowl pantry-check alignment, Ticket Pass / Prep Tray, or imagery.
...
| Planning pantry-count status lacks emphasis | The dynamic inventory fact in `Right now I see 17 pantry items we can work with.` reads like the surrounding helper sentence, even though it is the most useful status detail | Phase 3 added the empty-Pantry blocker/status line for behavior, then deferred final visual treatment to Phase 3.1 | Fixed in the pantry-count coral slice | Keep only the dynamic count phrase (`17 pantry items`, `1 pantry item`, or future `pantry ingredients` wording variants) in Planning coral. Keep the rest of the status sentence neutral and preserve empty-Pantry guard behavior |
...
- Planning entry highlights only the dynamic pantry-count phrase in the status line with Planning coral while preserving empty-Pantry guard behavior.
