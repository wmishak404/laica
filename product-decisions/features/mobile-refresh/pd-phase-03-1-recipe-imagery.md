...
Wilson clarified from mobile browser screenshots that the Chef It Up recipe-suggestion and Prep Tray issue is viewport fit inside browser chrome, not the accepted object proportions. The preferred fix is to shrink oversized typography on the affected interactable app surfaces instead of repositioning content or changing the ticket/prep composition.
...
- Scope type shrink to the MealPlanning runtime surfaces: Chef It Up time/cuisine/staples, Ticket Pass, and Prep Tray.
...
- Avoid collateral changes to landing/demo tickets or Slop Bowl headings unless separate validation evidence identifies those surfaces as browser-fit regressions.
...
| MealPlanning type overfills the mobile browser viewport | In mobile Chrome/Safari wrapper screenshots, Ticket Pass and Prep Tray text stayed proportionally attractive but too large for the visible browser viewport, forcing users to scroll around browser chrome and bottom controls | Phase 3.1 validated the ticket/prep object proportions mostly in app-like/full viewport contexts, while browser chrome reduces usable height | In progress in `codex/mobile-browser-type-fit` | Prefer scoped font-size reductions on MealPlanning headings, ticket labels, chips, and Prep Tray body copy over repositioning or changing the accepted Ticket Pass/Prep Tray proportions. Keep Live Cooking and Menu drawer typography unchanged unless Wilson separately reopens them |
