...
- Include Ready Check only as a pre-start action-fit exception; do not use that as permission to change active Live Cooking.
...
| Short planning/setup/result interactions overfill the mobile browser viewport | In mobile Chrome/Safari wrapper screenshots, Chef It Up time selection, cuisine, Ticket Pass, Prep Tray, first-time setup scanner, Ready Check, and Slop Bowl generated-result actions stayed proportionally attractive but too tall for the visible browser viewport, hiding useful controls behind browser/app bottom chrome | Phase 3.1/4 validation emphasized app-like/full viewport contexts, while browser chrome reduces usable height and `100vh` can exceed the actually visible browser area | In progress in `codex/mobile-browser-type-fit` | Prefer scoped `svh` sizing plus lower-action-lane fitting for short interactions, not blanket miniaturization. Time selection should place `Next` predictably low; cuisine should shorten the inner scroller so `No preference`/CTA remain reachable; Ticket Pass and Prep Tray should stay readable and only compact enough to expose their actions; setup scanner should start cleanly at the top; Slop Bowl generated-result action spacing should fit the plan-your-own-meal button where practical; Ready Check may compact as a pre-start exception so `Start cooking` is visible. Long Pantry/Settings inventory pages and the Slop Bowl pantry-check chip list are pass cases when they start at the top cleanly. Keep Live Cooking preparing/active surfaces and Menu drawer typography unchanged unless Wilson separately reopens them. Do not pin CTAs without explicit approval |
