...
Wilson clarified from mobile browser screenshots that the Chef It Up recipe-suggestion, Prep Tray, and Slop Bowl menu issue is viewport fit inside browser chrome, not the accepted object proportions. The preferred fix is to shrink oversized typography on the affected interactable app surfaces instead of repositioning content or changing the ticket/prep composition.
...
| Planning and Slop Bowl type overfill the mobile browser viewport | In mobile Chrome/Safari wrapper screenshots, Ticket Pass, Prep Tray, and Slop Bowl menu text stayed proportionally attractive but too large for the visible browser viewport, forcing users to scroll around browser chrome and bottom controls | Phase 3.1 validated the ticket/prep/Slop Bowl object proportions mostly in app-like/full viewport contexts, while browser chrome reduces usable height | In progress in `codex/mobile-browser-type-fit` | Prefer scoped font-size reductions on MealPlanning headings, ticket labels, chips, Prep Tray body copy, and Slop Bowl menu text/chips over repositioning or changing accepted proportions. Keep Live Cooking preparing/active surfaces and Menu drawer typography unchanged unless Wilson separately reopens them |
