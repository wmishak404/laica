...
Codex implemented the narrow Planning toast cleanup in PR #184. Guest setup-complete and returning guest profile-complete now share a title-only `Your kitchen is ready` toast with a 2.5-second duration. The toast no longer repeats browser/local retention language, so the Planning choice screen gets a quick confirmation without re-explaining guest persistence over the Chef It Up / Slop It Up cards.
