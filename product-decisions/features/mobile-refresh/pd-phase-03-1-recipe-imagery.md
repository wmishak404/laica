...
| Planning entry setup toast is too wordy and sticky | The post-setup `Your kitchen is ready` toast repeats anonymous/local retention explanation and stays over the primary Chef It Up / Slop It Up choice cards | INIT-003 added local guest-session semantics, but the transient Planning-entry confirmation inherited explanatory copy and persistence that belong in durable settings/onboarding context, not over the main choice surface | Deferred to Phase 3.1 | Remove the supporting line `I'll remember this on this browser while you try Laica.` from the setup-ready toast, keep any success confirmation concise, and make Planning-entry success/error/status messages auto-dismiss or otherwise avoid covering the primary cooking-path cards unless action is required |
