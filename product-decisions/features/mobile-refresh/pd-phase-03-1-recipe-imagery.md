...
| Short planning/setup/result interactions overfill the mobile browser viewport | In mobile Chrome/Safari wrapper screenshots, Chef It Up time selection, cuisine, Ticket Pass, Prep Tray, first-time setup scanner, and Slop Bowl generated-result actions stayed proportionally attractive but too tall for the visible browser viewport, hiding useful controls behind browser/app bottom chrome | Phase 3.1 validated these surfaces mostly in app-like/full viewport contexts, while browser chrome reduces usable height and `100vh` can exceed the actually visible browser area | In progress in `codex/mobile-browser-type-fit` | Prefer scoped `svh` sizing plus font-size/control reductions on MealPlanning headings, the time selector, cuisine/staple rows and fixed actions, ticket labels/chips/actions, Prep Tray body/CTA, setup scanner text/camera controls, and Slop Bowl generated-result action spacing over repositioning or changing accepted proportions. Long Pantry/Settings inventory pages and the Slop Bowl pantry-check chip list are pass cases when they start at the top cleanly. Keep Live Cooking preparing/active surfaces and Menu drawer typography unchanged unless Wilson separately reopens them |
