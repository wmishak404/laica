...
Wilson clarified from mobile browser screenshots that the Chef It Up setup/time, recipe-suggestion, Prep Tray, setup scanner, and Slop Bowl generated-result issue is viewport fit inside browser chrome, not the accepted object proportions. The preferred fix is to shrink oversized typography/controls and use the visible browser viewport on affected short interactions instead of repositioning content or changing the ticket/prep composition.

Clarified fitting rule: do not make every page tiny or force naturally long content into one screen. When the interaction is short enough to reasonably fit, useful controls should be visible in the same mobile-browser viewport before scrolling. Long pages/lists only need to start cleanly at the top and remain readable. Wilson explicitly treated the Pantry/Settings inventory page and Slop Bowl pantry-check chip list as pass examples under that rule.

2026-07-09 refinement: short decision pages should make the next/primary action predictable in the lower page area when content is light enough, but pinned primary actions are not approved yet and should be discussed before implementation. Cuisine selection is the exception where the inner option list should be visibly shorter/scrollable so `No preference` and the CTA stay discoverable. Ticket Pass and Prep Tray should not be over-shrunk just to fit; restore readable proportions when there is room. Ready Check is a pre-start decision screen and may get scoped fit/readability adjustments, but the active Live Cooking cockpit and preparing/loading guide remain excluded.

Later 2026-07-09 refinement from screenshots 12-16: Wilson approved trying a principled browser-mode system instead of further per-page pixel fitting. Top content should start below browser chrome with no hidden heading state; primary next actions float above the persistent app bottom nav; secondary actions remain in page flow; the app bottom nav may be visually shorter to make room for the primary action dock; floating back controls should not consume layout height but still need a collision-free title/header lane; comparable title/body/secondary text levels should stay consistent across adjacent pages; whitespace should breathe without shrinking core objects; and future native iOS layout should be allowed to diverge architecturally rather than inheriting browser-specific compromises. These principles are implemented on the branch as browser-mode direction and should be promoted to `design_guidelines.md` only after Wilson accepts the phone-browser visuals.

Later Slop Bowl refinement from screenshots 20-23: Slop Bowl should use the same browser-mode action/back pattern where it has short decision actions, but its pantry chip list remains naturally long content. The Slop Bowl pantry-check screen should default explicitly to 30 minutes rather than inheriting Chef It Up's last time setting, keep `Edit my pantry` inside the white pantry card, remove the redundant visible bottom `Back to options` once a floating back button exists, and float `Make my bowl` as the primary action. Slop Bowl generated-result and feedback screens should float only the primary action (`Let's cook this!` or `Recommend another bowl`) while secondary alternatives stay in flow. A Slop-origin Ready Check should route `Back to Planning` back to the Slop Bowl pantry menu, not Chef It Up's time screen.

Later setup-scanner refinement from screenshots 25-27: Pantry/Tools camera surfaces should stay camera-forward but not decorative at the expense of content. The camera-off/error message and bottom camera controls need separate vertical lanes so text does not overlap the toggle/capture/tips controls. Viewfinder corner brackets are not required on the setup/returning scan object unless they serve a concrete function, so omit them when they crowd the off-state. First-time setup screens also need a clean terminal scroll endpoint: when the user scrolls to the bottom, the page should stop at the Back/Next rail or its border instead of exposing inert blank space below it. Follow-up screenshots 29-32 showed that this endpoint rule must apply across mobile browser setup viewports, not only the shortest height breakpoint: the setup root should stay pinned to the visible viewport and the setup phone frame should be the scroll owner. This endpoint rule is now canonical in `design_guidelines.md`.
...
- The setup/returning camera object should not render decorative setup viewfinder corners in the off state, and its text/control lanes should not overlap in short browser viewports.
- First-time setup pages with a Back/Next rail should not scroll past that rail into inert blank space; the bottom rail is the page end unless a real fixed control requires padding. On mobile browsers, the setup root should be viewport-bound and the setup phone frame should own scrolling so the outer document cannot produce a blank tail.
- Include Slop Bowl pantry-check, generated-result, and feedback action-stack tightening, using Slop Bowl-specific wrappers rather than shared `.planning-screen` overrides. Keep Slop generation fixed to the 30-minute default in the Slop UI path.
- Do not force long Pantry/Settings inventory pages or the Slop Bowl pantry-check chip list into one viewport; those are pass cases when they start at the top cleanly.
- Include Ready Check only as a pre-start action-fit exception; do not use that as permission to change active Live Cooking. Do not pin active Live Cooking controls through the browser action-dock system.
...
- Do not use desktop Chrome/Replit browser screenshots as the visual acceptance gate for this issue unless Wilson re-approves that lane; phone-browser review is the useful visual check.
...
| Short planning/setup/result interactions overfill the mobile browser viewport | In mobile Chrome/Safari wrapper screenshots, Chef It Up time selection, cuisine, Ticket Pass, Prep Tray, first-time setup scanner, Ready Check, Slop Bowl actions, and returning-user admin surfaces stayed proportionally attractive but could feel too tall or typographically inconsistent for the visible browser viewport, hiding useful controls or making utilitarian Settings/History read like landing heroes | Phase 3.1/4 validation emphasized app-like/full viewport contexts, while browser chrome reduces usable height and `100vh` can exceed the actually visible browser area | In progress in `codex/mobile-browser-type-fit` | Prefer a scoped browser-mode layout system over blanket miniaturization. Short decision pages should start at the top, keep comparable text levels consistent, and float the primary next action above a shortened bottom nav while secondary actions remain in page flow. Do not reintroduce local oversized typography on shared primary actions; `.planning-primary-action` should compute to the same font size across Chef It Up, Slop It Up, and Ready Check. Time selection floats `Next`; cuisine keeps `No preference` in flow and floats the CTA; Ticket Pass floats `View prep tray` and leaves `Refresh suggestions` secondary; Prep Tray floats `Cook this`; setup scanner starts cleanly, keeps the camera off-state message and controls from overlapping, omits decorative setup viewfinder corners, and stops at the Back/Next rail instead of inert blank space; Slop Bowl pantry-check explicitly defaults to 30 minutes, keeps `Edit my pantry` in the card, removes the redundant visible bottom `Back to options`, and floats `Make my bowl`; Slop Bowl generated-result floats only `Let's cook this!`; Slop feedback floats `Recommend another bowl`; Ready Check may compact as a pre-start exception so `Start cooking` is visible and Slop-origin back returns to Slop Bowl. Long Pantry/Settings inventory pages should not be forced into one viewport, but browser-mode Pantry/Tools settings typography can be normalized when same-level headings, action rows, chips, inputs, or save buttons look larger than adjacent Planning surfaces. Settings hub, Cooking Profile, and History use the same returning-user browser typography scale because they are utilitarian/admin surfaces, while the public/pre-login landing hero remains excluded and can keep its larger display title. Keep Live Cooking preparing/active surfaces and Menu drawer typography unchanged unless Wilson separately reopens them. Known debt: the current floating back button can still overlap Chef It Up titles, so do not claim title-clearance victory until a future redesign resolves it. The no-inert-scroll-tail rule is now canonical in `design_guidelines.md`; promote broader browser-mode action-dock principles only after phone-browser visual acceptance |
