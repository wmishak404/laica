...
- Future scan-review chips may need session-state colors or indicators for `new from latest scan`, `already saved/found again`, and normal saved inventory. This should build on the Pantry/Kitchen chip palette without creating a noisy third design system. Product ownership currently remains in [EFF-014](../../../efforts/effort-014-scan-session-diff-and-duplicate-refinement.md) until a specific unclosed Mobile Refresh phase or implementation slice explicitly takes over the broader setup/Settings/latest-scan duplicate-review scope.
