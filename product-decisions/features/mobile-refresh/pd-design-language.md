...
- Future scan-review chips may need session-state colors or indicators for `new from latest scan`, `already saved/found again`, and normal saved inventory. This should build on the Pantry/Kitchen chip palette without creating a noisy third design system; keep that work inside INIT-001's Mobile Refresh phase records rather than reviving a standalone Effort unless the scope escapes the initiative.
