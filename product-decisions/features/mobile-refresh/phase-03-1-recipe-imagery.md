...
| Planning typography hierarchy split | The page headline, `Chef It Up` / `Slop Bowl` card titles, and short taglines felt like unrelated app surfaces | Design guidance allowed mood-based font swaps and treated hero-card taglines like dense UI support copy | Fixed with `Fraunces` card titles/taglines and `Patrick Hand` only for micro-banners/scribbles | Use one display family for primary decision objects; separate title/tagline through scale, weight, and shade before introducing another font |
...
- Review nearby mobile-refresh surfaces for typography drift: page titles, card titles, short hero-card taglines, body copy, chips, banners, and CTAs should use the same type ladder instead of swapping fonts by mood.
