...
| `MAKE GOOD SLOP` sticker was off-center | The sticker visibly drifted from the mockup alignment | Position was eyeballed from the right edge instead of centered by layout math | Fixed | Center tone stickers mechanically and verify in Replit screenshots |
| Planning labels lacked kitchen diary edge | Card type felt too generic and less cheffy/fun than the mockups | Design guidance lacked scoped handwritten-label rules for Planning | Fixed with scoped `Patrick Hand` labels | Keep handwriting to small tone labels only; validate readability on mobile |
