...
Branch `codex/init-001-assistance-inline-failure` implements the accepted Phase 4 criterion that cooking-assistance failures appear inside the relevant Live Cooking guidance area instead of relying on a transient toast or hidden caption text. When microphone access, recording/transcription, usage limits, empty assistance responses, or `/api/cooking/assistance` failures interrupt `Ask a question`, Live Cooking now shows a warm inline recovery panel below the step cues, keeps the pinned current step visible, keeps the bottom `Ask a question` control available for retry, and mirrors the issue into the caption transcript without sending those operational failures through speech synthesis.

The slice reuses the existing AI error classifier for assistance-route failures and keeps the status copy direct: the cook can keep following the current step and try asking again when ready. Retrying clears the inline issue, and a successful assistant answer clears it through the normal assistant-response path.
...
Local evidence: `npm ci`, `npx vitest run tests/unit/live-cooking-guest-session.test.tsx --testTimeout=15000`, `npm run check`, `npm run build`, `npm run test:unit`, `npm audit --audit-level=high`, and `git diff --check` pass. Focused assertions prove microphone-denial and assistance-route failures render the inline issue, preserve the current step, avoid toast-only presentation, and clear the issue on retry. PR #275 updated the guest E2E smoke for the new inline panel; exact-head GitHub `unit`, `e2e_guest_smoke`, `npm-audit`, `trufflehog_pr`, CodeQL, and Analyze checks passed at head `2d9c0b1`. Local targeted Playwright did not reach the changed assertion because the local dotenvx database is missing `anonymous_recipe_usage`, matching the known local-environment gap. Human Replit validation is deferred to release/batch validation unless Wilson asks for PR-level device microphone/provider smoke, because the branch is a narrow client failure-presentation slice with mocked provider coverage and no schema, auth, deployment, or persistence changes.
...
- Cooking-assistance failure appears in the relevant inline guidance/caption area, not only in a toast. Branch `codex/init-001-assistance-inline-failure` implements this with an inline retry panel while preserving the pinned current step.
