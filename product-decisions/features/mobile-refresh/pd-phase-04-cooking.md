...
The branch intentionally does not add `suggestedTimer` schema, timer kinds/reasons, provider prompt changes, route contracts, durable cooking-session schema changes, assistance failure handling, Finish/History semantics, formal eval work, or Phase 5 cleanup. Durable navigation scope is limited to showing the existing bottom nav on Ready Check. Focused local evidence includes `npx vitest run tests/unit/live-cooking-guest-session.test.tsx tests/unit/meal-planning.test.tsx tests/unit/planning-choice.test.tsx --testTimeout=15000`, `npx vitest run tests/unit/rate-limit.test.ts`, `npm run check`, `npm run build`, and `git diff --check`. Exact-head GitHub checks passed at `5abe6fe`; release/batch Replit validation remains deferred and should include the timer, compact CC row, visible `Time's up` state, raised speech quota after server restart, Prep Tray image fallback, and Ready Check navigation/nav behavior.
