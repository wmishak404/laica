...
- Timer controls are visible by default, can be hidden from the clock toggle beside CC, show the current recipe step's real duration in `H:MM:SS` format, and never auto-start.
