...
Branch `codex/init-001-phase4-timer-polish` starts from `origin/main` after PR #264 and its closeout. It implements the first bounded timer redesign without changing provider schema: Live Cooking shows a stable explicit-start five-minute timer control instead of preloading active timer state or exposing provider-duration suggestions. Navigating steps clears timer state, active timers can collapse to a compact time-only pill and expand back to controls, and the visible timer shows `0:05:00` with circular play/pause and reset controls. Timer controls are visible by default with a compact opt-out clock toggle beside CC. Timer speech copy now handles singular/plural duration text.
...
- Timer controls are visible by default, can be hidden from the clock toggle beside CC, show a default `0:05:00`, and never auto-start.
- The timer control uses circular play/pause and reset buttons; provider step durations are not surfaced as separate timer suggestions in this slice.
