...
Branch `codex/init-001-phase4-timer-polish` starts from `origin/main` after PR #264 and its closeout. It implements the first bounded timer redesign without changing provider schema: existing step `duration` values are treated as explicit-start timer suggestions, not preloaded active timer state. Navigating steps clears timer state, active timers can collapse to a compact time-only pill and expand back to controls, and obvious prep-only instructions such as chopping do not show step-specific duration suggestions even if a duration is present. Timer controls are now visible by default with a compact opt-out clock toggle beside CC; when no timer-worthy duration exists, the control offers a neutral 5-minute starter rather than inventing a step-specific cook time. Timer speech copy now handles singular/plural duration text.

Wilson's Replit review at PR head `3849c846` added a Ready Check app-shell follow-up that belongs in this same PR: Prep Tray selected-image loading now includes visible copy; Ready Check `Back to Planning` preserves and restores the Prep Tray / recipe suggestions instead of restarting planning; and the existing bottom nav remains visible on Ready Check per Wilson's explicit request, with active hands-busy cooking still hiding it. The planning session is dismissed only after `Start cooking` begins the active guide.
...
- Timer controls are visible by default, can be hidden from the clock toggle beside CC, and never auto-start.
- Suggested step-specific durations appear only on timer-worthy steps; steps without a timer-worthy duration use a neutral 5-minute starter if timers are visible.
