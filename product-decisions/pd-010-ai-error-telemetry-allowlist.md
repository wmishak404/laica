...
| `feature` | varchar(48) | no | Stable enum: `recipe_suggestions`, `chef_it_up_suggestions`, `slop_bowl_suggestions`, `cooking_steps`, `cooking_assistance`, `ingredient_detection`, `tts`, `tts_voices`, `transcription`. Legacy aliases `pantry_recipes` and `slop_bowl` may be read or accepted for compatibility, but new writes should use the canonical ids. |
...
| Generic JSONB `payload` column on `ai_error_events` | The whole point of EFF-019 is to prevent the first JSON-blob leak. A generic payload column has no enforcement boundary |
...
| Send to third-party analytics (Sentry, Datadog, etc.) | Out of v1 scope; would require a separate privacy review per the EFF-019 out-of-scope list |
