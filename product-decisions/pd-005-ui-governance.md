...
- Enforcement shipped through PR #64 / [EFF-015](../efforts/effort-015-ui-governance-enforcement.md): `.github/PULL_REQUEST_TEMPLATE.md` carries the reviewer gate for tone overrides, scoped-style reuse, mockup conformance, visible provenance cues, and validation-scope hygiene; `eslint.config.js` adds `laica-ui/no-token-hex-classname`, which rejects `className` matching `/bg-\[#|text-\[#|border-\[#/` through `npm run check`.
...
- Slop Bowl visual/token cleanup is no longer tracked as an active Effort; INIT-001 Phase 3.1 owns remaining Slop Bowl design alignment while the EFF-015 enforcement layer guards against reintroducing raw token-equivalent hex classes.
