# EPIC-015 — UI Governance Enforcement (Lint + PR Template)
...
- [PD-005 — UI Governance Operating Model](../product-decisions/005-ui-governance.md) — the rules this epic enforces
- [`design_guidelines.md`](../design_guidelines.md) — the visual standard the PR template must cite
- [EPIC-001 (resolved)](001-ui-governance.md) — historical context; this epic closes the resolution criterion EPIC-001 explicitly deferred ("enforcement mechanism shipped on `main`")
- [EPIC-016](016-slop-bowl-hex-literal-cleanup.md) — companion pilot-surface cleanup so the new rule passes on existing code

## Context

PD-005 captures the rubric (5 required + 3 recommended rules, primitive lock order, surface taxonomy, scoped-style reuse contract) but ships without an enforcement mechanism. EPIC-001's original resolution criterion 2 — "enforcement mechanism shipped on `main`" — was explicitly deferred from the graduation. This epic closes that loop.
...
- **PD-005 annotation** confirming enforcement is shipping (replacing the generic "file a narrow active epic" follow-up note).

### Out of scope

- ESLint rules for PD-005 rules 2 (Button `className` overrides) and 4 (`rounded-[N]` arbitrary radii). Could become PD-005 amendments or follow-up epics if hex-literal enforcement reveals they remain a high-volume drift class.
- Slop Bowl hex-literal cleanup — tracked separately as [EPIC-016](016-slop-bowl-hex-literal-cleanup.md).
...
4. This epic file has a final dated resolution note pointing at the merged PR(s).
