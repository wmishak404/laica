...
describe('Equipment vision prompts', () => {
  it('adds non-kitchen exclusions to the system prompt', () => {
    ...
    expect(systemPrompt).toContain('For tools stored in organizers, name the cooking tools when clearly visible, not the organizer. For example, list knives, wooden spoons, or spatulas, not the knife block, utensil crock, wire basket, or paper towel holder.');
  ...
  it('reinforces the exclusions in the user prompt', () => {
    ...
    expect(userPrompt).toContain('If knives, spoons, spatulas, or other cooking tools are stored inside a holder or block, list the tools when clearly visible, not the storage object. Do not list knife blocks, utensil crocks, wire baskets, or paper towel holders as equipment.');
