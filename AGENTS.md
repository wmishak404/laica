...
- `initiatives/INIT-004-ai-output-quality-evals.md` — read before changing AI output-quality eval rubrics, importing eval/open-coding data, human review labels, LLM-as-judge calibration, golden datasets, daily eval reports, prompt-candidate workflow, or recipe/Slop Bowl/cooking-step eval criteria. Durable eval discipline belongs in `docs/evals/workflow.md`; durable intake records belong in `docs/evals/registry.md` and `docs/evals/intakes/`, with INIT-004 linking to the current build context while active.
