...
**Planning-doc collaboration rule.** For planning artifacts such as `docs/handoffs/`, `product-decisions/`, `issues/`, ADRs, spec/intent docs, and workflow docs like `AGENTS.md` / `CLAUDE.md`:
...
**Open-issues rule.** The `issues/` directory tracks cross-cutting governance concerns that span features (see `issues/README.md`). Before starting any feature work that touches a governed domain, read the relevant open issue. Each issue's *Agent checklist* section lists the exact triggers. Current open issues:

- `issues/001-ui-governance.md` — read before adding new pages, tone-forward components, hex-literal styling, custom primitive overrides, or font/icon changes

If your work intersects with an open issue, cite it in your handoff and note how the change interacts (conforms / defers / adds new evidence). When the issue gains new signal from your work (new drift found, new surface added to a taxonomy), append a `## YYYY-MM-DD — <summary>` section to the issue file itself.
