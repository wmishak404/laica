...
**What changed overall rule.** Final task summaries, handoffs, and PR descriptions should start with a concise overall view, then still include the concrete implementation changelog, files changed, validation, and deferrals as before. State what changed across the product/workflow/docs system, why it matters for future coordination or merge readiness, what learning or discipline was added to future work, what was validated, and what remains deferred or unvalidated. If there is no broader system learning, say that plainly instead of inventing one.
