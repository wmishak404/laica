...
- The OAuth start preflight now has a masked production-domain target and the workflow no longer skips the scheduled job when private config is missing. Missing or rejected config should be visible as a preflight failure, while public logs hide raw target URLs, API keys, and provider diagnostics by default.
- A manual dispatch on final branch head `a4653fb` proved the workflow runs through checkout/setup/install and reaches the provider preflight command with target/key masking active. The live preflight still failed with a sanitized provider/config rejection, so the remaining issue is no longer an inert workflow; it is private identity-provider configuration alignment.
