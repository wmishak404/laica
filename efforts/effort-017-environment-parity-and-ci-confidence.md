...
Branch `codex/eff-017-linked-settings-coverage` extends the existing linked dev-auth E2E lane with a separate allowlisted `dev-test-linked-settings-ci` user. The new browser smoke signs in through Firebase custom-token dev auth, seeds a linked profile through the normal API, opens Settings from the real app menu, adds pantry and tools through the UI, saves both lists, and verifies the persisted linked profile through authenticated `/api/user/profile`.
