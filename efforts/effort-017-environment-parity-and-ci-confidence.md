...
- The OAuth start preflight now has a masked production-domain target and is manual-only while private provider/Firebase configuration remains blocked. Missing or rejected config is visible when manually dispatched, while public logs hide raw target URLs, API keys, and provider diagnostics by default. Restore a schedule only after the canary passes for the accepted stable target set.
- A manual dispatch after the masking correction proved the workflow runs through checkout/setup/install and reaches the provider preflight command with target/key masking active. The live preflight still failed with a sanitized provider/config rejection, so the remaining issue is no longer an inert workflow; it is private identity-provider configuration alignment.
