...
The final PR #201 linked browser smoke failure after the dismissal marker was a test harness issue, not a new product restore edge: Playwright's `page.addInitScript` persists across reloads, so the initial dev-auth script could reinsert an already-consumed Firebase custom token on later reloads and overwrite the fresh token queued for the final reload. The smoke now marks that one-time bootstrap in `sessionStorage`, leaving later reloads to use normal Firebase persistence or the explicitly queued fresh token.
