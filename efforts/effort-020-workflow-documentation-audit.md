# EFF-020 — Workflow documentation audit and graduation
...
During Mobile Refresh Phase 3.2 closeout, Wilson asked whether [EFF-005](effort-005-testing-strategy-and-acceptance-criteria.md) had become redundant because acceptance criteria and validation state now mostly live in more durable homes:
...
- Graduate [EFF-005](effort-005-testing-strategy-and-acceptance-criteria.md) once the testing workflow has a durable home.
...
|---|---|---|
| Testing and acceptance | [EFF-005](effort-005-testing-strategy-and-acceptance-criteria.md), `docs/workflows/replit-validation-focus.md`, feature phase records, `AGENTS.md`, ADR-0001 | EFF-005 still asks where criteria belong even though newer docs mostly answered it. Needs graduation to a central workflow doc or PD. |
...
| Database schema workflow | [EFF-010](effort-010-local-db-schema-strategy.md), `docs/workflows/environment-parity-spec.md`, [PD-008](../product-decisions/pd-008-optional-context-and-local-validation-boundaries.md) | Still active/open; keep schema workflow work with EFF-010 unless the audit only links to it. |
| Environment parity and CI confidence | [EFF-017](effort-017-environment-parity-and-ci-confidence.md), `docs/workflows/environment-parity-spec.md` | Deferred until after INIT-001; audit should not accidentally reactivate implementation scope. |

## Decisions made so far

- Do not treat EFF-005 as the long-term ledger for every feature's acceptance criteria.
- Do not resolve EFF-005 until its remaining useful policy has a durable home and references are repointed.
...
- Resolving, superseding, or substantially rewriting EFF-005.
...
1. The repo has a central, durable testing/acceptance workflow or process PD that answers EFF-005's remaining open questions.
2. EFF-005 is marked `Resolved`, removed from the active read list, and updated with a final resolution note pointing to the new durable artifact.
...
4. `efforts/README.md` and `efforts/registry.md` are updated for EFF-005 and EFF-020 status.
...
- [EFF-017 - Environment parity + CI confidence](effort-017-environment-parity-and-ci-confidence.md)
- [EFF-021 - Scan upload photo limit policy](effort-021-scan-upload-photo-limit-policy.md)
...
## 2026-05-08 - Corner-case testing methodology signal from EFF-021
...
Status changed from `Open` to `In Progress`. The audit has already mapped the existing workflow docs, captured the EFF-005 graduation path, and absorbed EFF-021's feature-impact and corner-case testing signals. The Effort remains unresolved until the central testing/acceptance workflow exists, EFF-005 is closed out, references agree, registry drift is fixed or explicitly deferred, and a final workflow-map handoff is pushed.
