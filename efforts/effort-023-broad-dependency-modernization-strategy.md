...
## 2026-07-20 - Multer runtime and declarations kept in one middleware slice

Dependabot PR #314 proposed updating only `@types/multer` from 2.0.0 to 2.2.0. Registry and lockfile inspection showed that npm's current stable runtime and declarations are both 2.2.0, while LAICA's old `multer` declaration still allowed 2.0.x even though the lockfile had already resolved 2.2.0. Branch `codex/deps-multer-2-2` therefore treats the runtime range, declarations, actual multipart behavior, and upload-error contract as one Express-middleware compatibility unit.

The focused slice exercises LAICA's real `/api/speech/transcribe` route with memory storage, the 10 MiB file-size boundary, provider-call exclusion on rejection, and the existing isolated temporary-file handoff. This is new evidence for the server-runtime grouping rule, but it does not start the other deferred modernization domains or change this Effort's `Deferred` status.
