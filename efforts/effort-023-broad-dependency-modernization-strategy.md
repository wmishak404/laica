...
- TruffleHog moves from `3.95.9` to `3.96.0` with both action refs and both explicit scanner-image inputs aligned. The upstream release labels a dependency security update, providing the concrete trigger that closed PR #338's incomplete proposal lacked: https://github.com/trufflesecurity/trufflehog/releases/tag/v3.96.0
- A daily high/critical npm audit checks the current lockfile even when no PR or push occurs, and a unit guard makes future wrapper/image version drift fail deterministically.
