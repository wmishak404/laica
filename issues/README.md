...
Long-lived design, architecture, and governance concerns tracked as markdown files. These are **agent-coordination artifacts**, not GitHub Issues. Claude and Codex must read relevant open issues before starting work in a governed domain.
...
- **Every time you start a new feature that touches a governed domain** — see each issue's *Agent checklist* section for the exact triggers
- Before writing a handoff that introduces patterns an open issue might constrain
- Before making a decision that might collide with something parked in the backlog

If your work intersects with an open issue, **cite the issue in your handoff** and note how your change interacts with it (conforms / defers / adds new signal).
...
| `issues/NNN-*.md` | **Open concerns / backlog / governance** | **Evolves** — updated as work progresses |

An issue can **graduate** to a product decision once resolution is clear. At that point, create the `PD-NNN-*.md`, flip the issue to `Resolved`, and add a pointer from the issue to the PD.

## Auto-push permission

Per `CLAUDE.md`, changes to `issues/*.md` follow the planning-doc collaboration rule — Claude and Codex may commit and push updates without asking, as long as the change is a progression of recorded context (new decisions, new evidence, status transitions, linked artifacts). Stop and ask Wilson when a change would:

- Flip an issue to `Resolved` without the resolution criteria being met
